 ...
 */
#include <arisenio/txn_test_gen_plugin/txn_test_gen_plugin.hpp>
...
FC_REFLECT(arisenio::detail::txn_test_gen_empty, );
FC_REFLECT(arisenio::detail::txn_test_gen_status, (status));
...
#define INVOKE_V_R_R_R(api_handle, call_name, in_param0, in_param1, in_param2) \
     ...
     arisenio::detail::txn_test_gen_status result = { status };

#define INVOKE_V_R_R(api_handle, call_name, in_param0, in_param1) \
     const auto& vs = fc::json::json::from_string(body).as<fc::variants>(); \
     api_handle->call_name(vs.at(0).as<in_param0>(), vs.at(1).as<in_param1>()); \
     arisenio::detail::txn_test_gen_empty result;
   ...
   [this](string, string body, url_response_callback cb) mutable { \
      ...
      auto result_handler = [times_called{std::move(times_called)}, cb, body](const fc::exception_ptr& e) mutable {\
         ...
         if (e) {\
            ...
            cb(http_response_code, fc::variant(arisenio::detail::txn_test_gen_empty())); \
         }\
      };\
      INVOKE \
   }\
...
struct txn_test_gen_plugin_impl {
   ...
   void create_test_accounts(const std::string& init_name, const std::string& init_priv_key, const std::function<void(const fc::exception_ptr&)>& next) {
      ...
      try {
         ...
         abi_def currency_abi_def = fc::json::from_string(contracts::arisenio_token_abi().data()).as<abi_def>();

         controller& cc = app().get_plugin<chain_plugin>().chain();
         auto chainid = app().get_plugin<chain_plugin>().get_chain_id();
         auto abi_serializer_max_time = app().get_plugin<chain_plugin>().get_abi_serializer_max_time();

         abi_serializer arisenio_token_serializer{fc::json::from_string(contracts::arisenio_token_abi().data()).as<abi_def>(), abi_serializer_max_time};
         ...
         //create some test accounts
         {
            ...
            //create "A" account
            {
            auto owner_auth   = arisenio::chain::authority{1, {{txn_text_receiver_A_pub_key, 1}}, {}};
            auto active_auth  = arisenio::chain::authority{1, {{txn_text_receiver_A_pub_key, 1}}, {}};
            ...
            //create "B" account
            {
            auto owner_auth   = arisenio::chain::authority{1, {{txn_text_receiver_B_pub_key, 1}}, {}};
            auto active_auth  = arisenio::chain::authority{1, {{txn_text_receiver_B_pub_key, 1}}, {}};
            ...
            //create "T" account
            {
            auto owner_auth   = arisenio::chain::authority{1, {{txn_text_receiver_C_pub_key, 1}}, {}};
            auto active_auth  = arisenio::chain::authority{1, {{txn_text_receiver_C_pub_key, 1}}, {}};
         ...
         //set newaccountT contract to arisenio.token & initialize it
         {
            signed_transaction trx;

            vector<uint8_t> wasm = contracts::arisenio_token_wasm();
...

            {
               setabi handler;
               handler.account = newaccountT;
               handler.abi = fc::raw::pack(json::from_string(contracts::arisenio_token_abi().data()).as<abi_def>());
               trx.actions.emplace_back( vector<chain::permission_level>{{newaccountT,"active"}}, handler);
            }

            {
               ...
               act.data = arisenio_token_serializer.variant_to_binary("create",
            ...
            }
            {
               ...
               act.name = N(issue);
               act.authorization = vector<permission_level>{{newaccountT,config::active_name}};
               act.data = arisenio_token_serializer.variant_to_binary("issue",
            ...
            }
            {
               ...
               act.name = N(transfer);
               act.authorization = vector<permission_level>{{newaccountT,config::active_name}};
               act.data = arisenio_token_serializer.variant_to_binary("transfer",
   ...
   string start_generation(const std::string& salt, const uint64_t& period, const uint64_t& batch_size) {
      ...
      act_a_to_b.data = arisenio_token_serializer.variant_to_binary("transfer",
      ...
      act_b_to_a.data = arisenio_token_serializer.variant_to_binary("transfer",
                                                                  fc::json::from_string(fc::format_string("{\"from\":\"${from}\",\"to\":\"${to}\",\"quantity\":\"1.0000 CUR\",\"memo\":\"${l}\"}",
