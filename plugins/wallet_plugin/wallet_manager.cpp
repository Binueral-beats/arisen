...
#include <arisenio/wallet_plugin/wallet.hpp>
#include <arisenio/wallet_plugin/se_wallet.hpp>
...
void wallet_manager::set_timeout(const std::chrono::seconds& t) {
   ...
   ARISEN_ASSERT(timeout_time >= now && timeout_time.time_since_epoch().count() > 0, invalid_lock_timeout_exception, "Overflow on timeout_time, specified ${t}, now ${now}, timeout_time ${timeout_time}",
...
std::string wallet_manager::create(const std::string& name) {
   ...
   ARISEN_ASSERT(valid_filename(name), wallet_exception, "Invalid filename, path not allowed in wallet name ${n}", ("n", name));

   auto wallet_filename = dir / (name + file_ext);

   if (fc::exists(wallet_filename)) {
      ARISEN_THROW(chain::wallet_exist_exception, "Wallet with name: '${n}' already exists at ${path}", ("n", name)("path",fc::path(wallet_filename)));
   }
...
void wallet_manager::open(const std::string& name) {
   ...
   if (!wallet->load_wallet_file()) {
      ARISEN_THROW(chain::wallet_nonexistent_exception, "Unable to open file: ${f}", ("f", wallet_filename.string()));
   }
...
map<public_key_type,private_key_type> wallet_manager::list_keys(const string& name, const string& pw) {
   ...
   if (wallets.count(name) == 0)
      ARISEN_THROW(chain::wallet_nonexistent_exception, "Wallet not found: ${w}", ("w", name));
   auto& w = wallets.at(name);
   if (w->is_locked())
      ARISEN_THROW(chain::wallet_locked_exception, "Wallet is locked: ${w}", ("w", name));
...
flat_set<public_key_type> wallet_manager::get_public_keys() {
   check_timeout();
   ARISEN_ASSERT(!wallets.empty(), wallet_not_available_exception, "You don't have any wallet!");
   ...
   ARISEN_ASSERT(!is_all_wallet_locked, wallet_locked_exception, "You don't have any unlocked wallet!");
   return result;
}
...
void wallet_manager::unlock(const std::string& name, const std::string& password) {
   ...
   auto& w = wallets.at(name);
   if (!w->is_locked()) {
      ARISEN_THROW(chain::wallet_unlocked_exception, "Wallet is already unlocked: ${w}", ("w", name));
      return;
   }
   w->unlock(password);
}
...
wallet_manager::sign_transaction(const chain::signed_transaction& txn, const flat_set<public_key_type>& keys, const chain::chain_id_type& id) {
   ...
   for (const auto& pk : keys) {
      ...
      if (!found) {
         ARISEN_THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets ${k}", ("k", pk));
      }
   }
...
wallet_manager::sign_digest(const chain::digest_type& digest, const public_key_type& key) {
   ...
   ARISEN_THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets ${k}", ("k", key));
}
...
void wallet_manager::own_and_use_wallet(const string& name, std::unique_ptr<wallet_api>&& wallet) {
   if(wallets.find(name) != wallets.end())
      ARISEN_THROW(wallet_exception, "Tried to use wallet name that already exists.");
   wallets.emplace(name, std::move(wallet));
}
   ...
   t->async_wait([t, this](const boost::system::error_code& /*ec*/)
   {
      ...
      if(ec != boost::system::error_code()) {
         if(rc.type() == bfs::file_not_found) {
            appbase::app().quit();
            ARISEN_THROW(wallet_exception, "Lock file removed while awalletd still running.  Terminating.");
         }
      }
...
void wallet_manager::initialize_lock() {
   ...
   lock_path = dir / "wallet.lock";
   {
      ...
      ARISEN_ASSERT(!x.fail(), wallet_exception, "Failed to open wallet lock file at ${f}", ("f", lock_path.string()));
   }
   ...
   if(!wallet_dir_lock->try_lock()) {
      ...
      ARISEN_THROW(wallet_exception, "Failed to lock access to wallet directory; is another awalletd running?");
   }
