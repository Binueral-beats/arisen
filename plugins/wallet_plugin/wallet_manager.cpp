...
std::string wallet_manager::create(const std::string& name) {
   ...
   // This can happen if the wallet file is removed while arisen-walletd is running.
...
void wallet_manager::open(const std::string& name) {
   ...
   // If we have name in our map then remove it since we want the emplace below to replace.
   // This can happen if the wallet file is added while arisen-walletd is running.
   auto it = wallets.find(name);
   if (it != wallets.end()) {
      wallets.erase(it);
   }
   ...
   t->async_wait([t, this](const boost::system::error_code& /*ec*/)
   {
      ...
      if(ec != boost::system::error_code()) {
         if(rc.type() == bfs::file_not_found) {
            appbase::app().quit();
            rsn_THROW(wallet_exception, "Lock file removed while krsnd still running.  Terminating.");
         }
      }
...
void wallet_manager::initialize_lock() {
   //This is technically somewhat racy in here -- if multiple krsnd are in this function at once.
   ...
   if(!wallet_dir_lock->try_lock()) {
      ...
      rsn_THROW(wallet_exception, "Failed to lock access to wallet directory; is another krsnd running?");
   }
