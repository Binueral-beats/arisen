#include <arisenio/history_plugin/history_plugin.hpp>
#include <arisenio/history_plugin/account_control_history_object.hpp>
#include <arisenio/history_plugin/public_key_history_object.hpp>
...
CHAINBASE_SET_INDEX_TYPE(arisenio::account_history_object, arisenio::account_history_index)
CHAINBASE_SET_INDEX_TYPE(arisenio::action_history_object, arisenio::action_history_index)
...
namespace arisenio {
      ...
      try {
         if( options.count( "filter-on" )) {
            ...
            for( auto& s : fo ) {
               ...
               ARISEN_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-on", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               ARISEN_ASSERT( fe.receiver.value, fc::invalid_arg_exception,
         ...
         if( options.count( "filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               ARISEN_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-out", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               ARISEN_ASSERT( fe.receiver.value, fc::invalid_arg_exception,
   ...
   namespace history_apis {
      read_only::get_actions_result read_only::get_actions( const read_only::get_actions_params& params )const {
        ...
        ARISEN_ASSERT( end >= start, chain::plugin_exception, "end position is earlier than start position" );
      ...
      read_only::get_transaction_result read_only::get_transaction( const read_only::get_transaction_params& p )const {
         ...
         } ARISEN_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", p.id))
         ...
         if( !in_history && !p.block_num_hint ) {
            ARISEN_THROW(tx_not_found, "Transaction ${id} not found in history and no block hint was given", ("id",p.id));
         }
         ...
         if( in_history ) {
            ...
            if (!found) {
               ARISEN_THROW(tx_not_found, "Transaction ${id} not found in history or in block number ${n}", ("id",p.id)("n", *p.block_num_hint));
            }
         }
