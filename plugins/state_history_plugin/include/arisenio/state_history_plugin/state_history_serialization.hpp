...
datastream<ST>& history_serialize_container(datastream<ST>& ds, const chainbase::database& db,
   ...
   fc::raw::pack(ds, unsigned_int(v.size()));
   for (auto& x : v) {
      RSN_ASSERT(!!x, arisenio::chain::plugin_exception, "null inside container");
      ds << make_history_serial_wrapper(db, *x);
   }
   return ds;
}
...
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<arisenio::chain::permission_object>& obj) {
   ...
   if (obj.obj.parent._id) {
      ...
      if (!parent) {
         ...
         RSN_ASSERT(it != undo.removed_values.end(), arisenio::chain::plugin_exception,
...
datastream<ST>& operator<<(datastream<ST>&                                                                      ds,
   ...
   RSN_ASSERT(!obj.obj.pending, arisenio::chain::plugin_exception,
