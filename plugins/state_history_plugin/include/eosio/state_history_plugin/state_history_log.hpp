...
#include <arisenio/chain/block_header.hpp>
...
class state_history_log {
 ...
 public:
   ...
   void read_header(state_history_log_header& header, bool assert_version = true) {
      ...
      ARISEN_ASSERT(!ds.remaining(), chain::plugin_exception, "state_history_log_header_serial_size mismatch");
      if (assert_version)
         ARISEN_ASSERT(is_ship(header.magic) && is_ship_supported_version(header.magic), chain::plugin_exception,
   ...
   void write_entry(const state_history_log_header& header, const chain::block_id_type& prev_id, F write_payload) {
      ...
      ARISEN_ASSERT(_begin_block == _end_block || block_num <= _end_block, chain::plugin_exception,
                 "missed a block in ${name}.log", ("name", name));

      if (_begin_block != _end_block && block_num > _begin_block) {
         if (block_num == _end_block) {
            ARISEN_ASSERT(prev_id == last_block_id, chain::plugin_exception, "missed a fork change in ${name}.log",
                       ("name", name));
         } else {
            state_history_log_header prev;
            get_entry(block_num - 1, prev);
            ARISEN_ASSERT(prev_id == prev.block_id, chain::plugin_exception, "missed a fork change in ${name}.log",
                       ("name", name));
         }
      }
      ...
      ARISEN_ASSERT(end == pos + state_history_log_header_serial_size + header.payload_size, chain::plugin_exception,
   ...
   std::fstream& get_entry(uint32_t block_num, state_history_log_header& header) {
      ARISEN_ASSERT(block_num >= _begin_block && block_num < _end_block, chain::plugin_exception,
 ...
 private:
   ...
   void recover_blocks(uint64_t size) {
      ...
      log.sync();
      ARISEN_ASSERT(get_last_block(pos), chain::plugin_exception, "recover ${name}.log failed", ("name", name));
   }
   ...
   void open_log() {
      ...
      if (size >= state_history_log_header_serial_size) {
         ...
         log.seekg(0);
         read_header(header, false);
         ARISEN_ASSERT(is_ship(header.magic) && is_ship_supported_version(header.magic) &&
         ...
         ARISEN_ASSERT(!size, chain::plugin_exception, "corrupt ${name}.log (5)", ("name", name));
   ...
   void open_index() {
      ...
      while (pos < size) {
         state_history_log_header header;
         ARISEN_ASSERT(pos + state_history_log_header_serial_size <= size, chain::plugin_exception,
         ...
         ARISEN_ASSERT(suffix == pos, chain::plugin_exception, "corrupt ${name}.log (8)", ("name", name));
...
FC_REFLECT(arisenio::state_history_log_header, (magic)(block_id)(payload_size))
