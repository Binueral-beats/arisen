...
datastream<ST>& history_serialize_container(datastream<ST>& ds, const chainbase::database& db,
   ...
   fc::raw::pack(ds, unsigned_int(v.size()));
   for (auto& x : v) {
      ARISEN_ASSERT(!!x, arisen::chain::plugin_exception, "null inside container");
      ds << make_history_serial_wrapper(db, *x);
   }
   return ds;
}
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<arisen::chain::producer_key>& obj) {
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.producer_name.value));
   fc::raw::pack(ds, as_type<arisen::chain::public_key_type>(obj.obj.block_signing_key));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                            ds,
                           const history_serial_wrapper<arisen::chain::shared_producer_schedule_type>& obj) {
   fc::raw::pack(ds, as_type<uint32_t>(obj.obj.version));
   history_serialize_container(ds, obj.db,
                               as_type<arisen::chain::shared_vector<arisen::chain::producer_key>>(obj.obj.producers));
   return ds;
}
...
datastream<ST>& operator<<(datastream<ST>&                                                     ds,
   ...
   fc::raw::pack(ds, make_history_serial_wrapper(
                         obj.db, as_type<arisen::chain::shared_producer_schedule_type>(obj.obj.proposed_schedule)));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<arisen::chain::key_weight>& obj) {
   fc::raw::pack(ds, as_type<arisen::chain::public_key_type>(obj.obj.key));
...
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<arisen::chain::permission_object>& obj) {
   ...
   if (obj.obj.parent._id) {
      ...
      if (!parent) {
         ...
         ARISEN_ASSERT(it != undo.removed_values.end(), arisen::chain::plugin_exception,
...
datastream<ST>& operator<<(datastream<ST>&                                                                      ds,
   ...
   ARISEN_ASSERT(!obj.obj.pending, arisen::chain::plugin_exception,
