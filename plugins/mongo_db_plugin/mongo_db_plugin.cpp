...
void mongo_db_plugin_impl::applied_transaction( const chain::transaction_trace_ptr& t ) {
   try {
      ...
      //
      // Allow logging traces if node is a producer for testing purposes, so a single nodrsn can do both for testing.
      //
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The target queue size between nodrsn and MongoDB plugin thread.")
