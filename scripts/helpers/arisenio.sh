...
if [[ -z "${NAME}" ]]; then
    ...
    else echo " ${COLOR_RED}- ARISENIO is not supported for your Architecture!${COLOR_NC}" && exit 1
    fi
fi

# Setup yum and apt variables
if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
    if ! YUM=$( command -v yum 2>/dev/null ); then echo "${COLOR_RED}YUM must be installed to compile ARISENIO${COLOR_NC}" && exit 1; fi
elif [[ $NAME == "Ubuntu" ]]; then
    if ! APTGET=$( command -v apt-get 2>/dev/null ); then echo "${COLOR_RED}APT-GET must be installed to compile ARISENIO${COLOR_NC}" && exit 1; fi
fi
...
function setup() {
    if $VERBOSE; then
        ...
        echo "ARISENIO_INSTALL_DIR: ${ARISENIO_INSTALL_DIR}"
...
function ensure-which() {
  if ! which ls &>/dev/null; then
    while true; do
      [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}ARISENIO compiler checks require the 'which' package: Would you like for us to install it? (y/n)?${COLOR_NC}" && read -p " " PROCEED
      echo ""
      case $PROCEED in
...
function install-directory-prompt() {
    if [[ -z $INSTALL_LOCATION ]]; then
        echo "No installation location was specified. Please provide the location where ARISENIO is installed."
        while true; do
            [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to use the default location? ${ARISENIO_INSTALL_DIR}? (y/n)${COLOR_NC}" && read -p " " PROCEED
            echo ""
            case $PROCEED in
                "" )
                echo "What would you like to do?";;
                ...
                break;;
                1 | false | [Nn]* )
                printf "Enter the desired installation location." && read -p " " ARISENIO_INSTALL_DIR;
                export ARISENIO_INSTALL_DIR;
                break;;
                * ) echo "Please type 'y' for yes or 'n' for no.";;
            esac
        done
        ...
        export ARISENIO_INSTALL_DIR="$INSTALL_LOCATION"
    fi
    . ./scripts/.build_vars
    echo "ARISENIO will be installed to: ${ARISENIO_INSTALL_DIR}"
}

function previous-install-prompt() {
  if [[ -d $ARISENIO_INSTALL_DIR ]]; then
    echo "ARISENIO has already been installed into ${ARISENIO_INSTALL_DIR}... It's suggested that you arisenio_uninstall.sh before re-running this script."
    while true; do
...
function resources() {
    echo "${COLOR_CYAN}ARISENIO website:${COLOR_NC} https://arisen.io"
    echo "${COLOR_CYAN}ARISENIO Telegram channel:${COLOR_NC} https://t.me/rsnProject"
    echo "${COLOR_CYAN}ARISENIO resources:${COLOR_NC} https://arisen.io/resources/"
    echo "${COLOR_CYAN}ARISENIO Stack Exchange:${COLOR_NC} https://arisenio.stackexchange.com"
}

function print_supported_linux_distros_and_exit() {
   echo "On Linux the ARISENIO build script only supports Amazon, Centos, and Ubuntu."
...
function ensure-cmake() {
    ...
    if [[ ! -e "${CMAKE}" ]]; then
        ...
        && ./bootstrap --prefix=${ARISENIO_INSTALL_DIR} \
        && make -j${JOBS} \
        && make install \
        && cd .. \
