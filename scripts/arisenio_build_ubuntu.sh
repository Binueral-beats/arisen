...
( [[ $NAME == "Ubuntu" ]] && ( [[ "$(echo ${VERSION_ID})" == "16.04" ]] || [[ "$(echo ${VERSION_ID})" == "18.04" ]] )  ) || ( echo " - You must be running 16.04.x or 18.04.x to install ARISENIO." && exit 1 )
...
ensure-apt-packages "${REPO_ROOT}/scripts/arisenio_build_ubuntu_deps" $(echo ${EXTRA_DEPS[@]})
echo ""
...
if $INSTALL_MONGO; then
	...
	if [[ ! -d $MONGO_C_DRIVER_ROOT ]]; then
		...
		&& mkdir -p cmake-build \
		&& cd cmake-build \
		&& $CMAKE -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$ARISENIO_INSTALL_DIR -DENABLE_BSON=ON -DENABLE_SSL=OPENSSL -DENABLE_AUTOMATIC_INIT_AND_CLEANUP=OFF -DENABLE_STATIC=ON -DENABLE_ICU=OFF -DENABLE_SNAPPY=OFF $PINNED_TOOLCHAIN .. \
		&& make -j${JOBS} \
		&& make install \
		&& cd ../.. \
	...
	if [[ ! -d $MONGO_CXX_DRIVER_ROOT ]]; then
		...
		&& cd build \
		&& $CMAKE -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$ARISENIO_INSTALL_DIR -DCMAKE_PREFIX_PATH=$ARISENIO_INSTALL_DIR $PINNED_TOOLCHAIN .. \
