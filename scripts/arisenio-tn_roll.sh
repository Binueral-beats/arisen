#!/bin/bash
#
# arisenio-tn_roll is used to have all of the instances of the RSN daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: arisenio-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./arisenio-tn_roll.sh is sufficient.
#

if [ -z "$arisenio_HOME" ]; then
    echo arisenio_HOME not set - $0 unable to proceed.
    exit -1
fi
...
prog=""
RD=""
for p in arisend ariseniod nodrsn; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for arisend or ariseniod or nodrsn
    exit 1
fi
...
if [ $DD = "all" ]; then
    for arisenio_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $arisenio_HOME/scripts/arisenio-tn_up.sh "$*"
    done
else
    bash $arisenio_HOME/scripts/arisenio-tn_up.sh "$*"
fi
unset arisenio_RESTART_DATA_DIR
