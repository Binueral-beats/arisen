#!/bin/bash
#
# arisenio-tn_roll is used to have all of the instances of the ARISEN daemon on a host brought down
...
prog=""
RD=""
for p in eosd ariseniod aos; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for eosd or ariseniod or aos
    exit 1
fi
