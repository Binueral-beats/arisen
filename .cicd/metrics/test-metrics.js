...
[
    'arisenio',
    'arisenio-base-images',
    'arisenio-beta',
    'arisenio-build-unpinned',
    'arisenio-debug',
    'arisenio-lrt',
    'arisenio-security'
];
...
async function getBuild(pipeline, buildNumber)
{
    ...
    const httpResponse = await fetch(`https://api.buildkite.com/v2/organizations/ARISENIO/pipelines/${pipeline}/builds/${buildNumber}${buildkiteAccessToken}`);
...
async function testMetrics(buildkiteObject)
{
    if (!isNullOrEmpty(buildkiteObject.type)) // input is a Buildkite job object
    {
        ...
        env.BUILDKITE_REPO = env.BUILDKITE_REPO.replace(new RegExp('^git@github.com:(ARISENIO/)?'), '').replace(new RegExp('.git$'), '');
        const metrics = [];
        const os = getOS(env);
