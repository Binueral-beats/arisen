  ...
  - label: "$(echo "$PLATFORM_JSON" | jq -r .ICON) $(echo "$PLATFORM_JSON" | jq -r .PLATFORM_NAME_FULL) - Build"
    ...
    plugins:
      - ARISENIO/anka#v0.5.7:
          ...
          pre-commands: 
            - "git clone git@github.com:ARISENIO/mac-anka-fleet.git && cd mac-anka-fleet && . ./ensure-tag.bash -u 12 -r 25G -a '-n'"
    env:
...
# trigger arisenio-sync-from-genesis for every build
if [[ "$BUILDKITE_PIPELINE_SLUG" == 'arisenio' && -z "${SKIP_INSTALL}${SKIP_LINUX}${SKIP_DOCKER}${SKIP_SYNC_TESTS}" ]]; then
    cat <<EOF
  - label: ":chains: Sync from Genesis Test"
    trigger: "arisenio-sync-from-genesis"
    async: false
...
# trigger arisenio-resume-from-state for every build
if [[ "$BUILDKITE_PIPELINE_SLUG" == 'arisenio' && -z "${SKIP_INSTALL}${SKIP_LINUX}${SKIP_DOCKER}${SKIP_SYNC_TESTS}" ]]; then
    cat <<EOF
  - label: ":outbox_tray: Resume from State Test"
    trigger: "arisenio-resume-from-state"
    async: false
    if: build.env("BUILDKITE_TAG") == null
    build:
      message: "Triggered by $BUILDKITE_PIPELINE_SLUG build $BUILDKITE_BUILD_NUMBER"
...
cat <<EOF
  ...
  - label: ":beer: Brew Updater"
    command: |
      buildkite-agent artifact download arisenio.rb . --step ':darwin: macOS 10.14 - Package Builder'
      buildkite-agent artifact upload arisenio.rb
    agents:
      queue: "automation-basic-builder-fleet"
    timeout: "${TIMEOUT:-5}"
