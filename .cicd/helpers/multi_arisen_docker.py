...
def get_commit_for_branch(branch):
    commit = None
    r = requests.get('https://api.github.com/repos/arisenio/arisen/git/refs/heads/{}'.format(branch))
...
def get_commit_for_tag(tag):
    commit = None
    r = requests.get('https://api.github.com/repos/arisenio/arisen/git/refs/tags/{}'.format(tag))
    if r.status_code == 200:
        commit = r.json().get('object').get('sha')
