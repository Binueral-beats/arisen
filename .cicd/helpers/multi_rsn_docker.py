...
def get_commit_for_branch(branch):
    commit = None
    r = requests.get('https://api.github.com/repos/ARISENIO/rsn/git/refs/heads/{}'.format(branch))
...
def get_commit_for_tag(tag):
    commit = None
    r = requests.get('https://api.github.com/repos/ARISENIO/rsn/git/refs/tags/{}'.format(tag))
    if r.status_code == 200:
        commit = r.json().get('object').get('sha')
...
if CURRENT_COMMIT:
    ...
    r = requests.get('https://api.buildkite.com/v2/organizations/ARISENIO/pipelines/arisenio/builds?commit={}'.format(CURRENT_COMMIT), headers=headers)
    if r.status_code == 200:
        resp = r.json()
        if resp:
            build = resp.pop(0)
...
for item in config.items('arisenio'):
...
for label, commit in commits.items():
    r = requests.get('https://api.buildkite.com/v2/organizations/ARISENIO/pipelines/arisenio/builds?commit={}'.format(commit), headers=headers)
    if r.status_code == 200:
        resp = r.json()
        if resp:
            build = resp.pop(0)
