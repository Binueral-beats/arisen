...
try:
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.arisenioAccount)
    ...
    contractDir="unittests/contracts/arisenio.token"
    wasmFile="arisenio.token.wasm"
    abiFile="arisenio.token.abi"
    ...
    if not enableMongo:
        ...
        if abiName != "account" or abiActionName != "close" or abiType != "close":
            errorExit("FAILURE - get ARISEN account failed", raw=True)
