...
def runNodarisenAndGetOutput(myTimeout=3):
...
try:
    ...
    for i in range(1,4):
        Print("Attempt %d." % (i))
        ret = runNodarisenAndGetOutput(timeout)
        assert(ret)
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killArisenInstances, False, keepLogs, killAll, dumpErrorDetails)
