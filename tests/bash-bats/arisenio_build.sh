...
SCRIPT_LOCATION="scripts/arisenio_build.sh"
TEST_LABEL="[arisenio_build]"

###################################################################
# ON MAC YOU NEED TO FULLY UNINSTALL ARISENIO BEFORE THESE WILL PASS #
###################################################################
...
@test "${TEST_LABEL} > Testing Arguments & Options" {
    ...
    if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
        ...
        run bash -c "printf \"y\ny\nn\nn\n\" | ./scripts/arisenio_build.sh"
        [[ ! -z $(echo "${output}" | grep "ARISENIO compiler checks require the 'which'") ]] || exit
    fi
    ...
    run bash -c "./arisenio_build.sh -y -P"
    ...
    # lack of -i
    [[ ! -z $(echo "${output}" | grep "ARISENIO_INSTALL_DIR: ${HOME}/arisenio/${ARISENIO_VERSION}") ]] || exit
    ## -o
    ...
    [[ ! -z $(echo "${output}" | grep "ARISENIO_INSTALL_DIR: /NEWPATH") ]] || exit
    [[ ! -z $(echo "${output}" | grep "TEMP_DIR: ${HOME}/tmp") ]] || exit
        ...
        [[ ! -z $(echo "${output}" | grep "ARISENIO_INSTALL_DIR: $TEMP_DIR/NEWPATH") ]] || exit
