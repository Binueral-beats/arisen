...
SCRIPT_LOCATION="scripts/arisenio_uninstall.sh"
TEST_LABEL="[arisenio_uninstall]"
...
@test "${TEST_LABEL} > Testing user prompts" {
  ...
  [[ $output =~ " - ARISENIO Removal Complete" ]] || exit
  ## First no shows "Cancelled..."
  run bash -c "echo \"n\" | ./$SCRIPT_LOCATION"
  [[ "${output##*$'\n'}" =~ "Cancelled ARISENIO Removal!" ]] || exit
...
@test "${TEST_LABEL} > -i" {
  run ./$SCRIPT_LOCATION -y -i ariseniotest
  [[ $output =~ .*/ariseniotest ]] || exit
  ([[ ! $output =~ "Library/Application\ Support/arisenio" ]] && [[ ! $output =~ ".local/share/arisenio" ]]) || exit
  [[ ! $output =~ "ARISENIO Removal Complete" ]] || exit
}
