...
export SCRIPT_LOCATION="scripts/arisenio_build.sh"
export TEST_LABEL="[arisenio_build_darwin]"
...
## Needed to load arisenio_build_ files properly; it can be empty
@test "${TEST_LABEL} > General" {
    ...
    [[ ! -z $(echo "${output}" | grep "Starting ARISENIO Dependency Install") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "brew tap arisenio/arisenio") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "Starting ARISENIO Build") ]] || exit
    [[ ! -z $(echo "${output}" | grep " --with-iostreams --with-date_time") ]] || exit # BOOST
    [[ ! -z $(echo "${output}" | grep "ARISENIO has been successfully built") ]] || exit
}
