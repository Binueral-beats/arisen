...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="arisenio-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is krsnd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=12618, walletHost="localhost", walletPort=12618, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="rsntest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet krsnd running. If not load the wallet plugin
        ...
        self.arisenioAccount=self.defProducerAccounts["arisenio"]= Account("arisenio")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodrsnArgs=" --http-max-response-time-ms 990000 ", useBiosBootFile=True, specificExtraNodrsnArgs=None, onlySetProds=False,
        ...
        extraNodrsnArgs: string of arguments to pass through to each nodoes instance (via --nodrsn flag on launcher)
        ...
        specificExtraNodrsnArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodrsn flags on launcher), example: { "5" : "--plugin arisenio::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the arisenio.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodrsn default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        nodrsnArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodrsnArgs += " --plugin arisenio::wallet_api_plugin"
        if self.enableMongo:
            nodrsnArgs += " --plugin arisenio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodrsnArgs is not None:
            assert(isinstance(extraNodrsnArgs, str))
            nodrsnArgs += extraNodrsnArgs
        if Utils.Debug:
            nodrsnArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodrsnArgs += " --plugin arisenio::producer_api_plugin"

        if nodrsnArgs:
            cmdArr.append("--nodrsn")
            cmdArr.append(nodrsnArgs)

        if specificExtraNodrsnArgs is not None:
            assert(isinstance(specificExtraNodrsnArgs, dict))
            for nodeNum,arg in specificExtraNodrsnArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodrsn")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodrsn")
                cmdArr.append(path)
        ...
        self.arisenioAccount=self.defProducerAccounts["arisenio"]
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.arisenioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse arisenio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by arisenio-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/arisenio.bios",
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/arisenio.bios"
        ...
        arisenioName="arisenio"
        arisenioKeys=producerKeys[arisenioName]
        arisenioAccount=Account(arisenioName)
        arisenioAccount.ownerPrivateKey=arisenioKeys["private"]
        arisenioAccount.ownerPublicKey=arisenioKeys["public"]
        arisenioAccount.activePrivateKey=arisenioKeys["private"]
        arisenioAccount.activePublicKey=arisenioKeys["public"]
        producerKeys.pop(arisenioName)

        if not self.walletMgr.importKey(arisenioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (arisenioName))
            return None
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"arisenio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init arisenio transfer")
            opts="--permission arisenio@active"
            if name != "arisenio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from arisenio.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        trans=biosNode.publishContract(arisenioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, arisenioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission arisenio@active"
                    myTrans=biosNode.pushMessage("arisenio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("arisenio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but arisenio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "arisenio"
        ...
        arisenioTokenAccount=copy.deepcopy(arisenioAccount)
        arisenioTokenAccount.name="arisenio.token"
        trans=biosNode.createAccount(arisenioTokenAccount, arisenioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (arisenioTokenAccount.name))
            return None

        arisenioRamAccount=copy.deepcopy(arisenioAccount)
        arisenioRamAccount.name="arisenio.ram"
        trans=biosNode.createAccount(arisenioRamAccount, arisenioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (arisenioRamAccount.name))
            return None

        arisenioRamfeeAccount=copy.deepcopy(arisenioAccount)
        arisenioRamfeeAccount.name="arisenio.rfee"
        trans=biosNode.createAccount(arisenioRamfeeAccount, arisenioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (arisenioRamfeeAccount.name))
            return None

        arisenioStakeAccount=copy.deepcopy(arisenioAccount)
        arisenioStakeAccount.name="arisenio.stake"
        trans=biosNode.createAccount(arisenioStakeAccount, arisenioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (arisenioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(arisenioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=arisenioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (arisenioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to arisenio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (arisenioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (arisenioAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to arisenio contract.")
            return None
        ...
        Utils.Print("Verify arisenio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountrsnBalanceStr(arisenioAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (arisenioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (arisenioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (arisenioTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (arisenioAccount.name)
            trans=biosNode.pushMessage(arisenioAccount.name, action, data, opts)
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodrsn instances. allInstances will kill all nodrsn instances running on the system."""
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodrsn instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodrsn instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
