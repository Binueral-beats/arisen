...
class Cluster(object):
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        extraNodrsnArgs: string of arguments to pass through to each nodoes instance (via --aos flag on launcher)
        ...
        specificExtraNodrsnArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-aos flags on launcher), example: { "5" : "--plugin arisenio::test_control_api_plugin" }
        ...
        if not self.walletd:
            aosArgs += " --plugin arisenio::wallet_api_plugin"
        if self.enableMongo:
            aosArgs += " --plugin arisenio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodrsnArgs is not None:
            assert(isinstance(extraNodrsnArgs, str))
            aosArgs += extraNodrsnArgs
        if Utils.Debug:
            aosArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            aosArgs += " --plugin arisenio::producer_api_plugin"
