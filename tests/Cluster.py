...
class Cluster(object):
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=12618, walletHost="localhost", walletPort=8889, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="ARISENtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        extraNodeosArgs: string of arguments to pass through to each nodoes instance (via --aos flag on launcher)
        ...
        if not self.walletd:
            nodeosArgs += " --plugin arisenio::wallet_api_plugin"
        if self.enableMongo:
            nodeosArgs += " --plugin arisenio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        ...
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodeosArgs += " --plugin arisenio::producer_api_plugin"
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        actualAmount=biosNode.getAccountEosBalanceStr(arisenioAccount.name)
    ...
    # Kills a percentange of Rsn instances starting from the tail and update eosInstanceInfos state
