...
class Cluster(object):
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodarisenArgs=" --http-max-response-time-ms 990000 ", useBiosBootFile=True, specificExtraNodarisenArgs=None, onlySetProds=False,
        ...
        extraNodarisenArgs: string of arguments to pass through to each nodoes instance (via --aos flag on launcher)
        ...
        specificExtraNodarisenArgs: dictionary of arguments to pass to a specific node (via --specific-num and
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.ArisenServerName))
            return True
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f %s --unstarted-nodes %s" % (
            Utils.ArisenLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            producerFlag, unstartedNodes)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodarisenArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodarisenArgs += " --plugin arisen::wallet_api_plugin"
        if self.enableMongo:
            nodarisenArgs += " --plugin arisen::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodarisenArgs is not None:
            assert(isinstance(extraNodarisenArgs, str))
            nodarisenArgs += extraNodarisenArgs
        if Utils.Debug:
            nodarisenArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodarisenArgs += " --plugin arisen::producer_api_plugin"

        if nodarisenArgs:
            cmdArr.append("--aos")
            cmdArr.append(nodarisenArgs)

        if specificExtraNodarisenArgs is not None:
            assert(isinstance(specificExtraNodarisenArgs, dict))
            for nodeNum,arg in specificExtraNodarisenArgs.items():
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.ArisenServerName, startedNodes, len(nodes)))
            return False
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.ArisenClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.ArisenClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextArisenIdx: %d, n: %d" % (nextArisenIdx, n))
                nextArisenIdx=(nextArisenIdx + 1)%count
                if not self.nodes[nextArisenIdx].killed:
                    #Utils.Print("nextArisenIdx: %d" % (nextArisenIdx))
            ...
            #Utils.Print("nextArisenIdx: %d, count: %d" % (nextArisenIdx, count))
            node=self.nodes[nextArisenIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.ArisenServerName, node.port))
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getArisenBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        ...
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/arisen.bios"
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        actualAmount=biosNode.getAccountArisenBalanceStr(arisenAccount.name)
    ...
    @staticmethod
    def pgrepArisenServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.ArisenServerName)
    ...
    @staticmethod
    def pgrepArisenServerPattern(nodeInstance):
        dataLocation=Utils.getNodeDataDir(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of ArisenInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepArisenServers(timeout)
        if psOut is None:
    ...
    def discoverLocalNode(self, nodeNum, psOut=None, timeout=None):
        ...
        if psOut is None:
            Utils.Print("ERROR: No nodes discovered.")
            return None
        pattern=Cluster.pgrepArisenServerPattern(nodeNum)
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.ArisenServerName, pattern))
            return None
    ...
    def discoverBiosNode(self, timeout=None):
        psOut=Cluster.pgrepArisenServers(timeout=timeout)
        pattern=Cluster.pgrepArisenServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.ArisenServerName, pattern))
    ...
    # Kills a percentange of Rsn instances starting from the tail and update arisenInstanceInfos state
    def killSomeArisenInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.ArisenServerName, killSignal))
    ...
    def relaunchArisenInstances(self, cachePopen=False):
    ...
    def killall(self, silent=True, allInstances=False):
        ...
        cmd="%s -k 9" % (Utils.ArisenLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        ...
        if allInstances:
            ...
            cmd="pkill -9 %s" % (Utils.ArisenServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
