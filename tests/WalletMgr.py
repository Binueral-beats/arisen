...
class WalletMgr(object):
    __walletLogOutFile="test_karisend_out.log"
    __walletLogErrFile="test_karisend_err.log"
    ...
    def __init__(self, walletd, nodarisenPort=12618, nodarisenHost="localhost", port=8889, host="localhost"):
        self.walletd=walletd
        self.nodarisenPort=nodarisenPort
        self.nodarisenHost=nodarisenHost
        self.port=port
        self.host=host
        self.wallets={}
    ...
    def getArgs(self):
        return " --url http://%s:%d%s %s" % (self.nodarisenHost, self.nodarisenPort, self.getWalletEndpointArgs(), Utils.MiscArisenClientArgs)
    ...
    def findAvailablePort(self):
        for i in range(WalletMgr.__MaxPort):
            ...
            if Utils.arePortsAvailable(port):
                return port
            if Utils.Debug: Utils.Print("Port %d not available for %s" % (port, Utils.ArisenWalletPath))

        Utils.errorExit("Failed to find free port to use for %s" % (Utils.ArisenWalletPath))
    ...
    def launch(self):
        ...
        pgrepCmd=Utils.pgrepCmd(Utils.ArisenWalletName)
        if Utils.Debug:
            portTaken=False
            if self.isLocal():
            ...
            if psOut or portTaken:
                ...
                Utils.Print("Launching %s, note similar processes running. %s" % (Utils.ArisenWalletName, statusMsg))

        cmd="%s --data-dir %s --config-dir %s --http-server-address=%s:%d --verbose-http-errors" % (
            Utils.ArisenWalletPath, WalletMgr.__walletDataDir, WalletMgr.__walletDataDir, self.host, self.port)
        ...
        try:
            if Utils.Debug: Utils.Print("Checking if %s launched. %s" % (Utils.ArisenWalletName, pgrepCmd))
            psOut=Utils.checkOutput(pgrepCmd.split())
            if Utils.Debug: Utils.Print("Launched %s. {%s}" % (Utils.ArisenWalletName, psOut))
    ...
    def create(self, name, accounts=None, exitOnError=True):
        ...
        cmd="%s %s %s --name %s --to-console" % (Utils.ArisenClientPath, self.getArgs(), cmdDesc, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=None
        maxRetryCount=4
        retryCount=0
    ...
    def importKey(self, account, wallet, ignoreDupKeyWarning=False):
        ...
        cmd="%s %s wallet import --name %s --private-key %s" % (
            Utils.ArisenClientPath, self.getArgs(), wallet.name, account.ownerPrivateKey)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
        ...
        if account.activePrivateKey is None:
            ...
            cmd="%s %s wallet import --name %s  --private-key %s" % (
                Utils.ArisenClientPath, self.getArgs(), wallet.name, account.activePrivateKey)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
                Utils.checkOutput(cmd.split())
            except subprocess.CalledProcessError as ex:
    ...
    def lockWallet(self, wallet):
        cmd="%s %s wallet lock --name %s" % (Utils.ArisenClientPath, self.getArgs(), wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def unlockWallet(self, wallet):
        cmd="%s %s wallet unlock --name %s" % (Utils.ArisenClientPath, self.getArgs(), wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def lockAllWallets(self):
        cmd="%s %s wallet lock_all" % (Utils.ArisenClientPath, self.getArgs())
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
    ...
    def getOpenWallets(self):
        ...
        cmd="%s %s wallet list" % (Utils.ArisenClientPath, self.getArgs())
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=None
        try:
            retStr=Utils.checkOutput(cmd.split())
    ...
    def getKeys(self, wallet):
        ...
        p = re.compile(r'\n\s+\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet private_keys --name %s --password %s " % (Utils.ArisenClientPath, self.getArgs(), wallet.name, wallet.password)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=None
        try:
            retStr=Utils.checkOutput(cmd.split())
    ...
    def killall(self, allInstances=False):
        ...
        if allInstances:
            cmd="pkill -9 %s" % (Utils.ArisenWalletName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
