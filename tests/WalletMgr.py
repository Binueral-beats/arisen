...
class WalletMgr(object):
    __walletLogOutFile="test_krsnd_out.log"
    __walletLogErrFile="test_krsnd_err.log"
    ...
    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch krsnd")
            return False
    ...
    def killall(self, allInstances=False):
        """Kill krsn instances. allInstances will kill all krsn instances running on the system."""
        if self.__walletPid:
