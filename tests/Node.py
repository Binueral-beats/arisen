...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, walletMgr=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="ARISENtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def arisenClientArgs(self):
        walletArgs=" " + self.walletMgr.getWalletEndpointArgs() if self.walletMgr is not None else ""
        return self.endpointArgs + walletArgs + " " + Utils.MiscArisenClientArgs
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.getArisenAccount(account.name, exitOnError=True)
            try:
    ...
    def getBlock(self, blockNum, silentErrors=False, exitOnError=False):
        ...
        if not self.enableMongo:
            ...
            return self.processClarisenCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError, exitMsg=msg)
        else:
    ...
    def getTransaction(self, transId, silentErrors=False, exitOnError=False, delayedRetry=True):
        ...
        if not self.enableMongo:
            ...
            for i in range(0,(int(60/timeout) - 1)):
                trans=self.processClarisenCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnErrorForDelayed, exitMsg=msg)
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False, stakeNet=100, stakeCPU=100, buyRAM=10000, exitOnError=False):
        ...
        trans=self.processClarisenCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getArisenAccount(self, name, exitOnError=False, returnType=ReturnType.json, avoidMongo=False):
        assert(isinstance(name, str))
        if not self.enableMongo or avoidMongo:
            cmdDesc="get account"
            jsonFlag="-j" if returnType==ReturnType.json else ""
            cmd="%s %s %s" % (cmdDesc, jsonFlag, name)
            msg="( getArisenAccount(name=%s) )" % (name);
            return self.processClarisenCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg, returnType=returnType)
        else:
            assert returnType == ReturnType.json, "MongoDB only supports a returnType of ReturnType.json"
            return self.getArisenAccountFromDb(name, exitOnError=exitOnError)

    def getArisenAccountFromDb(self, name, exitOnError=False):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def getTable(self, contract, scope, table, exitOnError=False):
        ...
        return self.processClarisenCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL, exitOnError=False):
        ...
        return self.processClarisenCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg, returnType=ReturnType.raw)
    ...
    def verifyAccount(self, account):
        assert(account)
        if not self.enableMongo:
            ret=self.getArisenAccount(account.name)
            if ret is not None:
    ...
    def verifyAccountMdb(self, account):
        assert(account)
        ret=self.getArisenAccountFromDb(account.name)
        if ret is not None:
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False, exitOnError=True):
        ...
        cmd="%s %s -v transfer -j %s %s" % (
            Utils.ArisenClientPath, self.arisenClientArgs(), source.name, destination.name)
        cmdArr=cmd.split()
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        currentBalances=self.getArisenBalances([source] + accounts)
    ...
    def getArisenBalances(self, accounts):
        ...
        balances={}
        for account in accounts:
            balance = self.getAccountArisenBalance(account.name)
    ...
    def getAccountArisenBalanceStr(self, scope):
        ...
        if Utils.Debug: Utils.Print("getNodeAccountArisenBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountArisenBalance(self, scope):
        """Returns RSN currency0000 account balance from arisecli get table command. Returned balance is an integer e.g. 980311. """
        balanceStr=self.getAccountArisenBalanceStr(scope)
        balance=Node.currencyStrToInt(balanceStr)
        return balance

    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.ArisenClientPath, self.arisenClientArgs(), account)
    ...
    def publishContract(self, account, contractDir, wasmFile, abiFile, waitForTransBlock=False, shouldFail=False):
        cmd="%s %s -v set contract -j %s %s" % (Utils.ArisenClientPath, self.arisenClientArgs(), account, contractDir)
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False):
        cmd="%s %s push action -j %s %s" % (Utils.ArisenClientPath, self.arisenClientArgs(), account, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False, exitOnError=False):
        ...
        trans=self.processClarisenCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError)
        self.trackCmdTransaction(trans)
    ...
    def delegatebw(self, fromAccount, netQuantity, cpuQuantity, toAccount=None, transferTo=False, waitForTransBlock=False, exitOnError=False):
        ...
        trans=self.processClarisenCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
        self.trackCmdTransaction(trans)
    ...
    def processClarisenCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        assert(isinstance(returnType, ReturnType))
        cmd="%s %s %s" % (Utils.ArisenClientPath, self.arisenClientArgs(), cmd)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def getInfo(self, silentErrors=False, exitOnError=False):
        cmdDesc = "get info"
        info=self.processClarisenCmd(cmdDesc, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError)
        if info is None:
    ...
    # If nodarisenPath is equal to None, it will use the existing aos path
    def relaunch(self, nodeId, chainArg=None, newChain=False, timeout=Utils.systemWaitTimeout, addOrSwapFlags=None, cachePopen=False, nodarisenPath=None):
        ...
        if nodarisenPath: splittedCmd[0] = nodarisenPath
    ...
    def getLatestBlockHeaderState(self):
        ...
        latestBlockHeaderState = self.processClarisenCmd(cmdDesc, cmdDesc)
