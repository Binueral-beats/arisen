...
class Node(object):
    ...
    def getBlock(self, blockNum, silentErrors=False, exitOnError=False):
        ...
        if not self.enableMongo:
            ...
            return self.processClrsnCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError, exitMsg=msg)
        else:
    ...
    def getTransaction(self, transId, silentErrors=False, exitOnError=False, delayedRetry=True):
        ...
        if not self.enableMongo:
            ...
            for i in range(0,(int(60/timeout) - 1)):
                trans=self.processClrsnCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnErrorForDelayed, exitMsg=msg)
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False, stakeNet=100, stakeCPU=100, buyRAM=10000, exitOnError=False):
        ...
        trans=self.processClrsnCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getrsnAccount(self, name, exitOnError=False, returnType=ReturnType.json, avoidMongo=False):
        ...
        if not self.enableMongo or avoidMongo:
            ...
            return self.processClrsnCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg, returnType=returnType)
        else:
    ...
    def getTable(self, contract, scope, table, exitOnError=False):
        ...
        return self.processClrsnCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL, exitOnError=False):
        ...
        return self.processClrsnCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg, returnType=ReturnType.raw)
    ...
    # Get actions mapped to an account (clrsn get actions)
    ...
    def getAccountrsnBalanceStr(self, scope):
        """Returns RSN currency0000 account balance from clrsn get table command. Returned balance is string following syntax "98.0311 RSN". """
        assert isinstance(scope, str)
        amount=self.getTableAccountBalance("arisenio.token", scope)
        if Utils.Debug: Utils.Print("getNodeAccountrsnBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountrsnBalance(self, scope):
        """Returns RSN currency0000 account balance from clrsn get table command. Returned balance is an integer e.g. 980311. """
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False, exitOnError=False):
        ...
        trans=self.processClrsnCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError)
        self.trackCmdTransaction(trans)
    ...
    def delegatebw(self, fromAccount, netQuantity, cpuQuantity, toAccount=None, transferTo=False, waitForTransBlock=False, exitOnError=False):
        ...
        trans=self.processClrsnCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
        self.trackCmdTransaction(trans)
    ...
    def processClrsnCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
    ...
    def getInfo(self, silentErrors=False, exitOnError=False):
        cmdDesc = "get info"
        info=self.processClrsnCmd(cmdDesc, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError)
        if info is None:
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by clrsn get info."""
        if not self.enableMongo:
    ...
    # If nodrsnPath is equal to None, it will use the existing nodrsn path
    def relaunch(self, nodeId, chainArg=None, newChain=False, timeout=Utils.systemWaitTimeout, addOrSwapFlags=None, cachePopen=False, nodrsnPath=None):
        ...
        if nodrsnPath: splittedCmd[0] = nodrsnPath
    ...
    # Require PREACTIVATE_FEATURE to be activated and require arisenio.bios with preactivate_feature
    def preactivateProtocolFeatures(self, featureDigests:list):
        for digest in featureDigests:
            ...
            trans=self.pushMessage("arisenio", "activate", data, opts)
    ...
    def getLatestBlockHeaderState(self):
        ...
        latestBlockHeaderState = self.processClrsnCmd(cmdDesc, cmdDesc)
