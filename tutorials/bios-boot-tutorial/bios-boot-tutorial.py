...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.arisecli + 'create account arisenio ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.arisecli + 'system newaccount --transfer arisenio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' % 
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.arisecli + 'transfer arisenio %s "%s"' % (a['name'], intToCurrency(unstaked)))
...
def claimRewards():
    table = getJsonOutput(args.arisecli + 'get table arisenio arisenio producers -l 100')
    times = []
...
def updateAuth(account, permission, parent, controller):
    run(args.arisecli + 'push action arisenio updateauth' + jsonArg({
        'account': account,
...
def msigReplaceSystem():
    run(args.arisecli + 'push action arisenio buyrambytes' + jsonArg(['arisenio', accounts[0]['name'], 200000]) + '-p arisenio')
    sleep(1)
...
def stepInstallSystemContracts():
    run(args.arisecli + 'set contract arisenio.token ' + args.contracts_dir + '/arisenio.token/')
    run(args.arisecli + 'set contract arisenio.msig ' + args.contracts_dir + '/arisenio.msig/')
def stepCreateTokens():
    run(args.arisecli + 'push action arisenio.token create \'["arisenio", "10000000000.0000 %s"]\' -p arisenio.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.arisecli + 'push action arisenio.token issue \'["arisenio", "%s", "memo"]\' -p arisenio' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    ...
    retry(args.arisecli + 'set contract arisenio ' + args.contracts_dir + '/arisenio.system/')
    sleep(1)

    # activate remaining features
    # GET_SENDER
    retry(args.arisecli + 'push action arisenio activate \'["f0af56d2c5a48d60a4a5b5c903edfb7db3a736a94ed589d0b797df33ff9d3e1d"]\' -p arisenio')
    # FORWARD_SETCODE
    retry(args.arisecli + 'push action arisenio activate \'["2652f5f96006294109b3dd0bbde63693f55324af452b799ee137a81a905eed25"]\' -p arisenio')
    # ONLY_BILL_FIRST_AUTHORIZER
    retry(args.arisecli + 'push action arisenio activate \'["8ba52fe7a3956c5cd3a656a3174b931d3bb2abb45578befc59f283ecd816a405"]\' -p arisenio')
    # RESTRICT_ACTION_TO_SELF
    retry(args.arisecli + 'push action arisenio activate \'["ad9e3d8f650687709fd68f4b90b41f7d825a365b02c23a636cef88ac2ac00c43"]\' -p arisenio')
    # DISALLOW_EMPTY_PRODUCER_SCHEDULE
    retry(args.arisecli + 'push action arisenio activate \'["68dcaa34c0517d19666e6b33add67351d8c5f69e999ca1e37931bc410a297428"]\' -p arisenio')
     # FIX_LINKAUTH_RESTRICTION
    retry(args.arisecli + 'push action arisenio activate \'["e0fb64b1085cc5538970158d05a009c24e276fb94e1a0bf6a528b48fbc4ff526"]\' -p arisenio')
     # REPLACE_DEFERRED
    retry(args.arisecli + 'push action arisenio activate \'["ef43112c6543b88db2283a2e077278c315ae2c84719a8b25f25cc88565fbea99"]\' -p arisenio')
    # NO_DUPLICATE_DEFERRED_ID
    retry(args.arisecli + 'push action arisenio activate \'["4a90c00d55454dc5b059055ca213579c6ea856967712a56017487886a4d4cc0f"]\' -p arisenio')
    # ONLY_LINK_TO_EXISTING_PERMISSION
    retry(args.arisecli + 'push action arisenio activate \'["1a99a59d87e06e09ec5b028a9cbb7749b4a5ad8819004365d02dc4379a8b7241"]\' -p arisenio')
    # RAM_RESTRICTIONS
    retry(args.arisecli + 'push action arisenio activate \'["4e7bf348da00a945489b2a681749eb56f5de00b900014e137ddae39f48f69d67"]\' -p arisenio')
    run(args.arisecli + 'push action arisenio setpriv' + jsonArg(['arisenio.msig', 1]) + '-p arisenio@active')

def stepInitSystemContract():
    run(args.arisecli + 'push action arisenio init' + jsonArg(['0', '4,' + args.symbol]) + '-p arisenio@active')
    sleep(1)
...
parser.add_argument('--public-key', metavar='', help="ARISENIO Public Key", default='ARISEN8Znrtgwt8TfpmbVpTKvA2oB8Nqey625CLN8bCN3TEbgx86Dsvr', dest="public_key")
