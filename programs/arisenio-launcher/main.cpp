...
#include <net/if.h>
#include <arisenio/chain/genesis_state.hpp>
...
using namespace arisenio::launcher::config;
...
struct server_name_def {
  ...
  string arisenio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), arisenio_home(), instances(1) {}
};
...
struct launcher_def {
   ...
   arisenio::chain::genesis_state genesis_from_file;
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &arisend : binding.instances) {
          arisend.host = binding.host_name;
          arisend.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(arisend.p2p_port);

          aliases.push_back (arisend.name);
        }
      }
  ...
  char* erd_env_var = getenv ("arisenio_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable arisenio_HOME is set to the build path." << endl;
    exit (-1);
  }
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.arisenio_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      rsnd_def arisend;
      assign_name(arisend, i == 0);
      aliases.push_back(arisend.name);
      arisend.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(arisend));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->arisenio_home =
          (local_id.contains (lhost->host_name) || servers.default_arisenio_home.empty()) ?
          erd : servers.default_arisenio_home;
        host_ndx++;
      } // ph_count == 0

      rsnd_def arisend;
      assign_name(arisend, do_bios);
      arisend.has_db = false;
      ...
      aliases.push_back(arisend.name);
      arisend.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(arisend));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->arisenio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->arisenio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->arisenio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->arisenio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->arisenio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = arisenio::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = arisenio::mongo_db_plugin\n";
  }
  cfg << "plugin = arisenio::net_plugin\n";
  cfg << "plugin = arisenio::chain_api_plugin\n"
...
launcher_def::init_genesis () {
   ...
   if (!bfs::exists(genesis_path)) {
      ...
      arisenio::chain::genesis_state default_genesis;
      fc::json::save_to_file( default_genesis, genesis_path, true );
   }
   string bioskey = string(network.nodes["bios"].keys[0].get_public_key());

   fc::json::from_file(genesis_path).as<arisenio::chain::genesis_state>(genesis_from_file);
...
launcher_def::prep_remote_config_dir (rsnd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->arisenio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->arisenio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->arisenio_home;

  cmd = "cd " + host->arisenio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->arisenio_home
...
launcher_def::launch (rsnd_def &instance, string &gts) {
  ...
  if (!rsnd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +arisenio::mongo_db_plugin");
  ...
  if (!host->is_local()) {
    ...
    string cmdl ("cd ");
    cmdl += host->arisenio_home + "; nohup " + rsndcmd + " > "
    ...
    string cmd = "cd " + host->arisenio_home + "; kill -15 $(cat " + pidf.string() + ")";
...
launcher_def::do_command(const host_def& host, const string& name,
   ...
   if (!host.is_local()) {
      string rcmd = "cd " + host.arisenio_home + "; ";
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      do_command(host, node.name, { { "arisenio_HOME", host.arisenio_home }, { "arisenio_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/arisenio-tn_down.sh ";
      do_command(host, node.name,
                 { { "arisenio_HOME", host.arisenio_home }, { "arisenio_NODE", node_num }, { "arisenio_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/arisenio-tn_roll.sh ";
      do_command(*host, host_name, { { "arisenio_HOME", host->arisenio_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the arisenio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the arisenio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the arisenio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << arisenio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(arisenio_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (arisenio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_arisenio_home) (ssh) )
