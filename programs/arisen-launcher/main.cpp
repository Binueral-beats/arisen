...
launcher_def::launch (eosd_def &instance, string &gts) {
  ...
  string eosdcmd = install_path + "programs/aos/" + string(node_executable_name) + " ";
  ...
  if (!host->is_local()) {
    ...
    string cmdl ("cd ");
    cmdl += host->arisenio_home + "; nohup " + eosdcmd + " > "
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/arisenio-tn_bounce.sh " + eosd_extra_args;
