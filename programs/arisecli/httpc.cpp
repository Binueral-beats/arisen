...
namespace arisenio { namespace client { namespace http {
   ...
   std::string do_txrx(T& socket, boost::asio::streambuf& request_buff, unsigned int& status_code) {
      ...
      ARISEN_ASSERT( status_code != 400, invalid_http_request, "The server has rejected the request as invalid!");

      std::string status_message;
      std::getline(response_stream, status_message);
      ARISEN_ASSERT( !(!response_stream || http_version.substr(0, 5) != "HTTP/"), invalid_http_response, "Invalid Response" );
      ...
      if( response_content_length != -1 ) {
         ...
         ARISEN_ASSERT(!ec || ec == boost::asio::ssl::error::stream_truncated, http_exception, "Unable to read http response: ${err}", ("err",ec.message()));
      }
   ...
   parsed_url parse_url( const string& server_url ) {
      ...
      if(res.scheme != "http" && res.scheme != "https")
         ARISEN_THROW(fail_to_resolve_host, "Unrecognized URL scheme (${s}) in URL \"${u}\"", ("s", res.scheme)("u", server_url));
      if(res.server.empty())
         ARISEN_THROW(fail_to_resolve_host, "No server parsed from URL \"${u}\"", ("u", server_url));
      if(res.port.empty())
   ...
   resolved_url resolve_url( const http_context& context, const parsed_url& url ) {
      ...
      if (ec) {
         ARISEN_THROW(fail_to_resolve_host, "Error resolving \"${server}:${port}\" : ${m}", ("server", url.server)("port",url.port)("m",ec.message()));
      }
      ...
      for(const auto& r : result) {
         ...
         if (resolved_port) {
            ARISEN_ASSERT(*resolved_port == port, resolved_to_multiple_ports, "Service name \"${port}\" resolved to multiple ports and this is not supported!", ("port",url.port));
         } else {
            resolved_port = port;
         }
      }
   ...
   if( status_code == 200 || status_code == 201 || status_code == 202 ) {
      ...
      auto &&error_info = response_result.as<arisenio::error_results>().error;
   ...
   ARISEN_ASSERT( status_code == 200, http_request_fail, "Error code ${c}\n: ${msg}\n", ("c", status_code)("msg", re) );
   return response_result;
   }
}}}
