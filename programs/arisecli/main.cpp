 ...
 *  @defgroup eosclienttool ARISENIO Command Line Client Reference
...
```
$ ./arisecli
Command Line Interface to ARISENIO Client
...
#include <arisenio/chain/config.hpp>
...
using namespace arisenio::client::help;
using namespace arisenio::client::http;
using namespace arisenio::client::localize;
using namespace arisenio::client::config;
using namespace boost::filesystem;
using auth_type = fc::static_variant<public_key_type, permission_level>;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define ARISENC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
string default_wallet_url = "unix://" + (determine_home_directory() / "arisenio-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
arisenio::client::http::http_context context;
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      auto sp = std::make_unique<arisenio::client::http::connection_param>(context, parse_url(url) + path, no_verify ? false : true, headers);
      return arisenio::client::http::do_http_call(*sp, fc::variant(v), print_request, print_response );
   }
...
arisenio::chain_apis::read_only::get_info_results get_info() {
   return call(url, get_info_func).as<arisenio::chain_apis::read_only::get_info_results>();
}
...
fc::variant push_transaction( signed_transaction& trx, packed_transaction::compression_type compression = packed_transaction::none ) {
   ...
   if (trx.signatures.size() == 0) { // #5445 can't change txn content if already signed
      ...
      } ARISEN_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
auto abi_serializer_resolver = [](const name& account) -> fc::optional<abi_serializer> {
   ...
   if ( it == abi_cache.end() ) {
      ...
      auto abi_results = result.as<arisenio::chain_apis::read_only::get_abi_results>();
...
bytes json_or_file_to_bin( const account_name& account, const action_name& action, const string& data_or_filename ) {
   ...
   if( !data_or_filename.empty() ) {
      ...
      } ARISEN_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data_or_filename));
   }
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } ARISEN_RETHROW_EXCEPTIONS(authority_type_exception, "Fail to parse Authority JSON '${data}'", ("data",authorityJsonOrFile))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "ARISEN") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } ARISEN_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
      auto result = parse_json_authority(authorityJsonOrFile);
      ARISEN_ASSERT( arisenio::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}

asset to_asset( account_name code, const string& s ) {
   static map< pair<account_name, arisenio::chain::symbol_code>, arisenio::chain::symbol> cache;
   auto a = asset::from_string( s );
   arisenio::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<arisenio::chain_apis::read_only::get_currency_stats_result>();
         auto p = cache.emplace( make_pair( code, sym ), result.max_supply.get_symbol() );
         it = p.first;
      } else {
         ARISEN_THROW(symbol_type_exception, "Symbol ${s} is not supported by token contract ${c}", ("s", sym_str)("c", code));
      }
   }
   ...
   if ( a.decimals() < expected_symbol.decimals() ) {
      ...
      ARISEN_THROW(symbol_type_exception, "Too many decimal digits in ${a}, only ${d} supported", ("a", a)("d", expected_symbol.decimals()));
   } // else precision matches
   return a;
}

inline asset to_asset( const string& s ) {
   return to_asset( N(arisenio.token), s );
}
...
struct set_account_permission_subcommand {
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->add_flag("--add-code", add_code, localized("[code] add '${code}' permission to specified permission authority", ("code", name(config::arisenio_code_name))));
      permissions->add_flag("--remove-code", remove_code, localized("[code] remove '${code}' permission from specified permission authority", ("code", name(config::arisenio_code_name))));

      add_standard_transaction_options(permissions, "account@active");

      permissions->set_callback([this] {
         ARISENC_ASSERT( !(add_code && remove_code), "ERROR: Either --add-code or --remove-code can be set" );
         ARISENC_ASSERT( (add_code ^ remove_code) || !authority_json_or_file.empty(), "ERROR: authority should be specified unless add or remove code permission" );
         ...
         if ( need_parent || need_auth ) {
            ...
            auto res = json.as<arisenio::chain_apis::read_only::get_account_results>();
            ...
            if ( need_auth ) {
               ...
               auto code_name = name(config::arisenio_code_name);
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->set_callback([this] {
         ...
         } ARISEN_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}", ("public_key", producer_key_str))
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      createAccount->set_callback([this] {
            ...
            if( owner_key_str.find('@') != string::npos ) {
               ...
               } ARISEN_RETHROW_EXCEPTIONS( explained_exception, "Invalid owner permission level: ${permission}", ("permission", owner_key_str) )
            } else {
               try {
                  owner = public_key_type(owner_key_str);
               } ARISEN_RETHROW_EXCEPTIONS( public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str) );
            }
            ...
            if( active_key_str.empty() ) {
               ...
               } ARISEN_RETHROW_EXCEPTIONS( explained_exception, "Invalid active permission level: ${permission}", ("permission", active_key_str) )
            } else {
               try {
                  active = public_key_type(active_key_str);
               } ARISEN_RETHROW_EXCEPTIONS( public_key_type_exception, "Invalid active public key: ${public_key}", ("public_key", active_key_str) );
            }
...
struct vote_producers_subcommand {
   string voter_str;
   vector<arisenio::name> producer_names;
...
struct approve_producer_subcommand {
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            ...
            ARISEN_ASSERT( 1 == res.rows.size(), multiple_voter_info, "More than one voter_info for account" );
            auto prod_vars = res.rows[0]["producers"].get_array();
            vector<arisenio::name> prods;
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         ...
         auto result = rawResult.as<arisenio::chain_apis::read_only::get_producers_result>();
...
struct get_transaction_id_subcommand {
   ...
   get_transaction_id_subcommand(CLI::App* actionRoot) {
      ...
      get_transaction_id->set_callback([&] {
         ...
         } ARISEN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_check))
      });
   }
};
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->set_callback([this] {
         ...
         ARISENC_ASSERT( !(buy_ram_amount.size()) || !buy_ram_bytes, "ERROR: --buyram and --buy-ram-bytes cannot be set at the same time" );
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->set_callback([this] {
         ARISENC_ASSERT( !kbytes || !bytes, "ERROR: --kbytes and --bytes cannot be set at the same time" );
         if (kbytes || bytes) {
...
void get_account( const string& accountName, const string& coresym, bool json_format ) {
   ...
   if (!json_format) {
      ...
      unordered_map<name, arisenio::chain_apis::permission> cache;
      ...
      if( res.refund_request.is_object() ) {
         ...
         if( unstaking > asset( 0, unstaking.get_symbol() ) ) {
            ...
            if( now >= refund_time ) {
               std::cout << " (available to claim now with 'arisenio::refund' action)\n";
            } else {
...
int main( int argc, char** argv ) {
   ...
   context = arisenio::client::http::create_http_context();
   wallet_url = default_wallet_url;

   CLI::App app{"Command Line Interface to ARISENIO Client"};
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", arisenio::client::config::version_str)) << std::endl;
   });
   ...
   pack_transaction->set_callback([&] {
      ...
      } ARISEN_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to parse plain transaction JSON '${data}'", ("data", plain_signed_transaction_json))
      if( pack_action_data_flag ) {
         ...
         } ARISEN_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to convert transaction, --pack-action-data likely needed" )
      }
   });
   ...
   unpack_transaction->set_callback([&] {
      ...
      } ARISEN_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to parse packed transaction JSON '${data}'", ("data", packed_transaction_json))
   ...
   pack_action_data->set_callback([&] {
      ...
      } ARISEN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse unpacked action data JSON")
   ...
   unpack_action_data->set_callback([&] {
      ARISEN_ASSERT( packed_action_data_string.size() >= 2, transaction_type_exception, "No packed_action_data found" );
   ...
   getAccounts->set_callback([&] {
      ...
      } ARISEN_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", public_key_str))
   ...
   getTransactions->set_callback([&] {
      ...
      if (skip_seq_str.empty()) {
         ...
         } ARISEN_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Skip Seq: ${skip_seq}", ("skip_seq", skip_seq_str))
         if (num_seq_str.empty()) {
            ...
            } ARISEN_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Num Seq: ${num_seq}", ("num_seq", num_seq_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        ARISEN_ASSERT( !wasm.empty(), wasm_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   auto set_abi_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        ARISEN_ASSERT( fc::exists( abiPath ), abi_file_not_found, "no abi file found ${f}", ("f", abiPath)  );
      ...
      if (!duplicate) {
         ...
         } ARISEN_RETHROW_EXCEPTIONS(abi_type_exception,  "Fail to parse ABI JSON")
         if ( shouldSend ) {
   ...
   contractSubcommand->set_callback([&] {
      if(!contract_clear) ARISEN_ASSERT( !contractPath.empty(), contract_exception, " contract-dir is null ", ("f", contractPath) );
      shouldSend = false;
      set_code_callback();
      set_abi_callback();
      if (actions.size()) {
   ...
   createWallet->set_callback([&wallet_name, &password_file, &print_console] {
      ARISENC_ASSERT( !password_file.empty() ^ print_console, "ERROR: Either indicate a file using \"--file\" or pass \"--to-console\"" );
      ARISENC_ASSERT( password_file.empty() || !std::ofstream(password_file.c_str()).fail(), "ERROR: Failed to create file in specified path" );
   ...
   importWallet->set_callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         ...
         ARISEN_THROW(private_key_type_exception, "Invalid private key: ${private_key}", ("private_key", wallet_key_str))
      }
   ...
   removeKeyWallet->set_callback([&wallet_name, &wallet_pw, &wallet_rm_key_str] {
      ...
      try {
         ...
         ARISEN_THROW(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", wallet_rm_key_str))
      }
   ...
   actionsSubcommand->set_callback([&] {
      ...
      if( !data.empty() ) {
         ...
         } ARISEN_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data))
      }
   ...
   trxSubcommand->set_callback([&] {
      ...
      } ARISEN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_push))
      try {
         signed_transaction trx = trx_var.as<signed_transaction>();
   ...
   trxsSubcommand->set_callback([&] {
      ...
      } ARISEN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trxsJson))
   ...
   propose_action->set_callback([&] {
      ...
      } ARISEN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",requested_perm))
      fc::variant transaction_perm_var;
      try {
         transaction_perm_var = json_from_file_or_string(transaction_perm);
      } ARISEN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",transaction_perm))
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string(proposed_transaction);
      } ARISEN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",proposed_transaction))
      ...
      } ARISEN_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_perm_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } ARISEN_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            ARISEN_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "arisenio.msig", "propose", variant_to_bin( N(arisenio.msig), N(propose), args ) }});
   });
   ...
   review->set_callback([&] {
      ...
      std::map<arisenio::account_name, std::pair<fc::time_point, vector<decltype(all_approvals)::iterator>>>  provided_approvers;
      ...
      if( show_approvals_in_multisig_review ) {
         ...
         if( new_multisig ) {
            for( auto& a : provided_approvers ) {
               ...
               if( rows4.empty() || rows4[0].get_object()["account"].as<arisenio::name>() != a.first ) {
                  continue;
               }
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      } ARISEN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",perm))
      ...
      send_actions({chain::action{accountPermissions, "arisenio.msig", action, variant_to_bin( N(arisenio.msig), action, args ) }});
   };
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "arisenio.msig", "invalidate", variant_to_bin( N(arisenio.msig), "invalidate", args ) }});
   });
   ...
   cancel->set_callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            ARISEN_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "arisenio.msig", "cancel", variant_to_bin( N(arisenio.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            ARISEN_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "arisenio.msig", "exec", variant_to_bin( N(arisenio.msig), N(exec), args ) }});
      }
   );
   ...
   wrap_exec->set_callback([&] {
      ...
      } ARISEN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_exec))
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send arisenio.system contract action to the blockchain."), false);
