...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "RSN") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      ...
      ARISEN_ASSERT( arisen::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}
      ...
      permissions->set_callback([this] {
         ...
         if ( need_parent || need_auth ) {
            ...
            if ( need_auth ) {
               ...
               auto code_name = name(config::ARISEN_code_name);
...
//                                ->check(CLI::ExistingFile);
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "arisen.msig", "invalidate", variant_to_bin( N(arisen.msig), "invalidate", args ) }});
   });
