...
Usage: programs/clrsn/clrsn [OPTIONS] SUBCOMMAND
...
//copy pasta from krsnd's main.cpp
...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "RSN") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      ...
      RSN_ASSERT( arisenio::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}
...
void ensure_krsnd_running(CLI::App* app) {
    if (no_auto_krsnd)
        return;
    // get, version, net do not require krsnd
    ...
    // This extra check is necessary when running clrsn like this: ./clrsn ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if clrsn and krsnd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("krsnd").append(key_store_executable_name);
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        ::boost::process::child krsn(binPath, pargs,
                                     bp::std_in.close(),
                                     bp::std_out > bp::null,
                                     bp::std_err > bp::null);
        if (krsn.running()) {
...
int main( int argc, char** argv ) {
   ...
   app.add_flag( "--no-auto-" + string(key_store_executable_name), no_auto_krsnd, localized("don't automatically launch a ${k} if one is not currently running", ("k", key_store_executable_name)));
   app.set_callback([&app]{ ensure_krsnd_running(&app);});
...
//                                ->check(CLI::ExistingFile);
   ...
   auto stopKrsnd = wallet->add_subcommand("stop", localized("Stop ${k}.", ("k", key_store_executable_name)), false);
   stopKrsnd->set_callback([] {
      const auto& v = call(wallet_url, krsnd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success krsnd responds with empty object
