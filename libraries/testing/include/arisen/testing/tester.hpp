...
#define REQUIRE_MATCHING_OBJECT(left, right) { auto a = fc::variant( left ); auto b = fc::variant( right ); BOOST_REQUIRE_EQUAL( true, a.is_object() ); \
   BOOST_REQUIRE_EQUAL( true, b.is_object() ); \
   auto filtered = ::arisenio::testing::filter_fields( a.get_object(), b.get_object() ); \
...
namespace arisenio { namespace testing {
  ...
  /**
   * Utility predicate to check whether an arisenio_assert message is equivalent to a given string
   */
  struct arisenio_assert_message_is {
     arisenio_assert_message_is( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }

     bool operator()( const arisenio_assert_message_exception& ex );

     string expected;
  };

  /**
   * Utility predicate to check whether an arisenio_assert message starts with a given string
   */
  struct arisenio_assert_message_starts_with {
     arisenio_assert_message_starts_with( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }
  ...
  /**
   * Utility predicate to check whether an arisenio_assert_code error code is equivalent to a given number
   */
  struct arisenio_assert_code_is {
     arisenio_assert_code_is( uint64_t error_code )
           : expected( "assertion failure with error code: " ) {
        expected.append( std::to_string(error_code) );
     }

     bool operator()( const arisenio_assert_code_exception& ex );
