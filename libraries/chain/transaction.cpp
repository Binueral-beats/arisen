...
void deferred_transaction_generation_context::reflector_init() {
      ...
      ARISEN_ASSERT( sender != account_name(), ill_formed_deferred_transaction_generation_context,
...
void transaction_header::validate()const {
   ARISEN_ASSERT( max_net_usage_words.value < UINT32_MAX / 8UL, transaction_exception,
...
{ try {
   ...
   for(const signature_type& sig : signatures) {
      ...
      ARISEN_ASSERT( sig_start < deadline, tx_cpu_usage_exceeded, "transaction signature verification executed for too long",
      ...
      ARISEN_ASSERT( allow_duplicate_keys || successful_insertion, tx_duplicate_sig,
...
vector<transaction_extensions> transaction::validate_and_extract_extensions()const {
   ...
   static_assert( std::is_same<transaction_extensions_t, arisenio::chain::transaction_extensions>::value,
   ...
   for( size_t i = 0; i < transaction_extensions.size(); ++i ) {
      ...
      ARISEN_ASSERT( id >= id_type_lower_bound, invalid_transaction_extension,
      ...
      ARISEN_ASSERT( match, invalid_transaction_extension,
                  "Transaction extension with id type ${id} is not supported",
                  ("id", id)
      );

      if( match->enforce_unique ) {
         ARISEN_ASSERT( i == 0 || id > id_type_lower_bound, invalid_transaction_extension,
...
uint32_t packed_transaction::get_unprunable_size()const {
   ...
   ARISEN_ASSERT( size <= std::numeric_limits<uint32_t>::max(), tx_too_big, "packed_transaction is too big" );
...
struct read_limiter {
   ...
   {
      ARISEN_ASSERT(_total + count <= Limit, tx_decompression_error, "Exceeded maximum decompressed transaction size");
      _total += count;
...
bytes packed_transaction::get_raw_transaction() const
{
   try {
      switch(compression) {
         ...
         default:
            ARISEN_THROW(unknown_transaction_compression, "Unknown transaction compression algorithm");
      }
...
void packed_transaction::reflector_init()
{
   ...
   ARISEN_ASSERT( unpacked_trx.expiration == time_point_sec(), tx_decompression_error, "packed_transaction already unpacked" );
   local_unpack_transaction({});
   local_unpack_context_free_data();
}
...
{
   try {
      ARISEN_ASSERT(unpacked_trx.context_free_data.empty(), tx_decompression_error, "packed_transaction.context_free_data not empty");
      switch( compression ) {
         case none:
