...
namespace arisenio { namespace chain {
   ...
   void block_log::open(const fc::path& data_dir) {
      ...
      if (log_size) {
         ...
         ARISEN_ASSERT( my->version > 0, block_log_exception, "Block log was not setup properly" );
         ARISEN_ASSERT( my->version >= min_supported_version && my->version <= max_supported_version, block_log_unsupported_version,
         ...
         if (my->version > 1){
            ...
            ARISEN_ASSERT(my->first_block_num > 0, block_log_exception, "Block log is malformed, first recorded block number is 0 but must be greater than or equal to 1");
         } else {
   ...
   uint64_t block_log::append(const signed_block_ptr& b) {
      try {
         ARISEN_ASSERT( my->genesis_written_to_block_log, block_log_append_fail, "Cannot append to block log until the genesis is first written" );
         ...
         ARISEN_ASSERT(my->index_stream.tellp() == sizeof(uint64_t) * (b->block_num() - my->first_block_num),
   ...
   signed_block_ptr block_log::read_block_by_num(uint32_t block_num)const {
      try {
         ...
         if (pos != npos) {
            ...
            ARISEN_ASSERT(b->block_num() == block_num, reversible_blocks_exception,
   ...
   fc::path block_log::repair_log( const fc::path& data_dir, uint32_t truncate_at_block ) {
      ...
      ARISEN_ASSERT( fc::is_directory(data_dir) && fc::is_regular_file(data_dir / "blocks.log"), block_log_not_found,
      ...
      ARISEN_ASSERT( blocks_dir_name.generic_string() != ".", block_log_exception, "Invalid path to blocks directory" );
      backup_dir = backup_dir / blocks_dir_name.generic_string().append("-").append( now );

      ARISEN_ASSERT( !fc::exists(backup_dir), block_log_backup_dir_exist,
      ...
      ARISEN_ASSERT( version > 0, block_log_exception, "Block log was not setup properly" );
      ARISEN_ASSERT( version >= min_supported_version && version <= max_supported_version, block_log_unsupported_version,
                 "Unsupported version of block log. Block log version is ${version} while code supports version(s) [${min},${max}]",
      ...
      if (version != 1) {
         ...
         ARISEN_ASSERT(actual_totem == expected_totem, block_log_exception,
