...
#define N(X) arisenio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<arisenio::chain::name> : private hash<uint64_t> {
      typedef arisenio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const arisenio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, arisenio::chain::name& check);
} // fc


FC_REFLECT( arisenio::chain::name, (value) )
