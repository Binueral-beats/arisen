...
namespace arisenio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(arisenio);
const static uint64_t null_account_name      = N(arisenio.null);
const static uint64_t producers_account_name = N(arisenio.prods);
...
const static uint64_t arisenio_auth_scope       = N(arisenio.auth);
const static uint64_t arisenio_all_scope        = N(arisenio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t arisenio_any_name = N(arisenio.any);
const static uint64_t arisenio_code_name = N(arisenio.code);
...
const static arisenio::chain::wasm_interface::vm_type default_wasm_runtime = arisenio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace arisenio::chain::config

constexpr uint64_t ARISEN_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / arisenio::chain::config::percent_100;
}

template<typename Number>
Number ARISEN_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + arisenio::chain::config::percent_100 - arisenio::chain::config::percent_1)  / arisenio::chain::config::percent_100;
}
