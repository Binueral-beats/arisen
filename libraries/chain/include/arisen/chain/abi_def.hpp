...
abi_def arisenio_contract_abi(const abi_def& arisenio_system_abi);
...
datastream<ST>& operator << (datastream<ST>& s, const arisenio::chain::may_not_exist<T>& v) {
   raw::pack(s, v.value);
   return s;
}

template<typename ST, typename T>
datastream<ST>& operator >> (datastream<ST>& s, arisenio::chain::may_not_exist<T>& v) {
   if (s.remaining())
...
template<typename T>
void to_variant(const arisenio::chain::may_not_exist<T>& e, fc::variant& v) {
   to_variant( e.value, v);
}

template<typename T>
void from_variant(const fc::variant& v, arisenio::chain::may_not_exist<T>& e) {
   from_variant( v, e.value );
}

} // namespace fc

FC_REFLECT( arisenio::chain::type_def                         , (new_type_name)(type) )
FC_REFLECT( arisenio::chain::field_def                        , (name)(type) )
FC_REFLECT( arisenio::chain::struct_def                       , (name)(base)(fields) )
FC_REFLECT( arisenio::chain::action_def                       , (name)(type)(ricardian_contract) )
FC_REFLECT( arisenio::chain::table_def                        , (name)(index_type)(key_names)(key_types)(type) )
FC_REFLECT( arisenio::chain::clause_pair                      , (id)(body) )
FC_REFLECT( arisenio::chain::error_message                    , (error_code)(error_msg) )
FC_REFLECT( arisenio::chain::variant_def                      , (name)(types) )
FC_REFLECT( arisenio::chain::abi_def                          , (version)(types)(structs)(actions)(tables)
