...
namespace arisenio { namespace chain {
    ...
    *  In the context of arisenio, every action is dispatched to the handler defined
   ...
   struct action {
      ...
      template<typename T>
      T data_as()const {
         ARISEN_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         ARISEN_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
...
FC_REFLECT( arisenio::chain::permission_level, (actor)(permission) )
FC_REFLECT( arisenio::chain::action, (account)(name)(authorization)(data) )
