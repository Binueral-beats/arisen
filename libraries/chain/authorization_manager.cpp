...
#include <arisenio/chain/authority_checker.hpp>
...
#include <arisenio/chain/database_utils.hpp>
...
namespace arisenio { namespace chain {
   ...
   namespace detail {
      ...
      struct snapshot_row_traits<permission_object> {
         ...
         static void from_snapshot_row(snapshot_permission_object&& row, permission_object& value, chainbase::database& db) {
            ...
            value.parent = 0;
            if (value.id == 0) {
               ARISEN_ASSERT(row.parent == permission_name(), snapshot_exception, "Unexpected parent name on reserved permission 0");
               ARISEN_ASSERT(row.name == permission_name(), snapshot_exception, "Unexpected permission name on reserved permission 0");
               ARISEN_ASSERT(row.owner == name(), snapshot_exception, "Unexpected owner name on reserved permission 0");
               ARISEN_ASSERT(row.auth.accounts.size() == 0,  snapshot_exception, "Unexpected auth accounts on reserved permission 0");
               ARISEN_ASSERT(row.auth.keys.size() == 0,  snapshot_exception, "Unexpected auth keys on reserved permission 0");
               ARISEN_ASSERT(row.auth.waits.size() == 0,  snapshot_exception, "Unexpected auth waits on reserved permission 0");
               ARISEN_ASSERT(row.auth.threshold == 0,  snapshot_exception, "Unexpected auth threshold on reserved permission 0");
               ARISEN_ASSERT(row.last_updated == time_point(),  snapshot_exception, "Unexpected auth last updated on reserved permission 0");
               value.parent = 0;
               ...
               ARISEN_ASSERT(parent.id != 0, snapshot_exception, "Unexpected mapping to reserved permission 0");
   ...
   { try {
      ARISEN_ASSERT( !level.actor.empty() && !level.permission.empty(), invalid_permission, "Invalid permission" );
      return _db.find<permission_object, by_owner>( boost::make_tuple(level.actor,level.permission) );
   } ARISEN_RETHROW_EXCEPTIONS( chain::permission_query_exception, "Failed to retrieve permission: ${level}", ("level", level) ) }
                                                                             ...
                                                                             )const
   {
      ...
      if( scope == config::system_account_name ) {
          ARISEN_ASSERT( act_name != updateauth::get_name() &&
      ...
      try {
         ...
         if( *linked_permission == config::arisenio_any_name )
            return optional<permission_name>();
   ...
   {
      ARISEN_ASSERT( auths.size() == 1, irrelevant_auth_exception,
                  "updateauth action should only have one declared authorization" );
      const auto& auth = auths[0];
      ARISEN_ASSERT( auth.actor == update.account, irrelevant_auth_exception,
      ...
      ARISEN_ASSERT( get_permission(auth).satisfies( *min_permission,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      ARISEN_ASSERT( auth.actor == del.account, irrelevant_auth_exception,
                                                           ...
                                                           )const
   {
      ...
      const auto& auth = auths[0];
      ARISEN_ASSERT( auth.actor == link.account, irrelevant_auth_exception,
      ...
      {
         ARISEN_ASSERT( link.type != updateauth::get_name(),  action_validate_exception,
                     "Cannot link arisenio::updateauth to a minimum permission" );
         ARISEN_ASSERT( link.type != deleteauth::get_name(),  action_validate_exception,
                     "Cannot link arisenio::deleteauth to a minimum permission" );
         ARISEN_ASSERT( link.type != linkauth::get_name(),    action_validate_exception,
                     "Cannot link arisenio::linkauth to a minimum permission" );
         ARISEN_ASSERT( link.type != unlinkauth::get_name(),  action_validate_exception,
                     "Cannot link arisenio::unlinkauth to a minimum permission" );
         ARISEN_ASSERT( link.type != canceldelay::get_name(), action_validate_exception,
                     "Cannot link arisenio::canceldelay to a minimum permission" );
      }

      const auto linked_permission_name = lookup_minimum_permission(link.account, link.code, link.type);

      if( !linked_permission_name ) // if action is linked to arisenio.any permission
         return;

      ARISEN_ASSERT( get_permission(auth).satisfies( get_permission({link.account, *linked_permission_name}),
                                                  _db.get_index<permission_index>().indices()              ),
                  irrelevant_auth_exception,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      ARISEN_ASSERT( auth.actor == unlink.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      const auto unlinked_permission_name = lookup_linked_permission(unlink.account, unlink.code, unlink.type);
      ARISEN_ASSERT( unlinked_permission_name.valid(), transaction_exception,
      ...
      if( *unlinked_permission_name == config::arisenio_any_name )
         return;

      ARISEN_ASSERT( get_permission(auth).satisfies( get_permission({unlink.account, *unlinked_permission_name}),
                                                  _db.get_index<permission_index>().indices()                  ),
                  irrelevant_auth_exception,
                                                                          ...
                                                                          )const
   {
      ...
      ARISEN_ASSERT( get_permission(auth).satisfies( get_permission(cancel.canceling_auth),
                                                  _db.get_index<permission_index>().indices() ),
                  irrelevant_auth_exception,
      ...
      ARISEN_ASSERT( itr != generated_index.end() && itr->sender == account_name() && itr->trx_id == trx_id,
                  tx_not_found,
      ...
      ARISEN_ASSERT( found, action_validate_exception,
                                             ...
                                             )const
   {
      ...
      for( const auto& act : actions ) {
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is arisenio.any
                  const auto& min_permission = get_permission({declared_auth.actor, *min_permission_name});
                  ARISEN_ASSERT( get_permission(declared_auth).satisfies( min_permission,
                                                                       _db.get_index<permission_index>().indices() ),
                              irrelevant_auth_exception,
      ...
      for( const auto& p : permissions_to_satisfy ) {
         ...
         ARISEN_ASSERT( checker.satisfied( p.first, p.second ), unsatisfied_authorization,
      ...
      if( !allow_unused_keys ) {
         ARISEN_ASSERT( checker.all_keys_used(), tx_irrelevant_sig,
                                             ...
                                             )const
   {
      ...
      ARISEN_ASSERT( checker.satisfied({account, permission}), unsatisfied_authorization,
                                                                     ...
                                                                     )const
   {
      ...
      for (const auto& act : trx.actions ) {
         for (const auto& declared_auth : act.authorization) {
            ARISEN_ASSERT( checker.satisfied(declared_auth), unsatisfied_authorization,
