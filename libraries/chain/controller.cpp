...
#include <arisenio/chain/block_log.hpp>
#include <arisenio/chain/fork_database.hpp>
#include <arisenio/chain/exceptions.hpp>

#include <arisenio/chain/account_object.hpp>
#include <arisenio/chain/code_object.hpp>
#include <arisenio/chain/block_summary_object.hpp>
...
#include <arisenio/chain/contract_table_objects.hpp>
#include <arisenio/chain/generated_transaction_object.hpp>
#include <arisenio/chain/transaction_object.hpp>
#include <arisenio/chain/reversible_block_object.hpp>
#include <arisenio/chain/genesis_intrinsics.hpp>
#include <arisenio/chain/whitelisted_intrinsics.hpp>

#include <arisenio/chain/protocol_feature_manager.hpp>
#include <arisenio/chain/authorization_manager.hpp>
#include <arisenio/chain/resource_limits.hpp>
#include <arisenio/chain/chain_snapshot.hpp>
...
struct controller_impl {
   ...
   void pop_block() {
      ...
      if( !prev ) {
         ARISEN_ASSERT( fork_db.root()->id == head->header.previous, block_validate_exception, "attempt to pop beyond last irreversible block" );
         prev = fork_db.root();
      }
      ...
      if ( read_mode == db_read_mode::SPECULATIVE ) {
         ARISEN_ASSERT( head->block, block_validate_exception, "attempting to pop a block that was sparsely loaded from a snapshot");
   ...
   inline void set_activation_handler() {
      ...
      ARISEN_ASSERT( res.second, misc_exception, "attempting to set activation handler twice" );
   }
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( arisenio, arisenio, newaccount );
   SET_APP_HANDLER( arisenio, arisenio, setcode );
   SET_APP_HANDLER( arisenio, arisenio, setabi );
   SET_APP_HANDLER( arisenio, arisenio, updateauth );
   SET_APP_HANDLER( arisenio, arisenio, deleteauth );
   SET_APP_HANDLER( arisenio, arisenio, linkauth );
   SET_APP_HANDLER( arisenio, arisenio, unlinkauth );
/*
   SET_APP_HANDLER( arisenio, arisenio, postrecovery );
   SET_APP_HANDLER( arisenio, arisenio, passrecovery );
   SET_APP_HANDLER( arisenio, arisenio, vetorecovery );
*/

   SET_APP_HANDLER( arisenio, arisenio, canceldelay );
   }
   ...
   void log_irreversible() {
      ARISEN_ASSERT( fork_db.root(), fork_database_exception, "fork database not properly initialized" );
      ...
      if( log_head ) {
         ARISEN_ASSERT( root_id == log_head->id(), fork_database_exception, "fork database root does not match block log head" );
      } else {
         ARISEN_ASSERT( fork_db.root()->block_num == lib_num, fork_database_exception,
   ...
   void replay(std::function<bool()> shutdown) {
      ...
      if( start_block_num <= blog_head->block_num() ) {
         ...
         if( pending_head->block_num < head->block_num || head->block_num < fork_db.root()->block_num ) {
            ...
            ARISEN_ASSERT( new_root, fork_database_exception, "unexpected error: could not find new LIB in fork database" );
   ...
   void init(std::function<bool()> shutdown, const snapshot_reader_ptr& snapshot) {
      ...
      if( snapshot ) {
         ...
         if( db.revision() < 1 || !fork_db.head() ) {
            if( fork_db.head() ) {
               ...
               ARISEN_ASSERT( db.revision() < 1, database_exception,
            ...
            if( blog.head() ) {
               ARISEN_ASSERT( blog.first_block_num() == 1, block_log_exception,
            ...
            if( blog.head() ) {
               ARISEN_ASSERT( first_block_num <= lib_num && lib_num <= blog.head()->block_num(),
                           block_log_exception,
      ...
      ARISEN_ASSERT( db.revision() >= head->block_num, fork_database_exception,
      ...
      if( read_mode == db_read_mode::IRREVERSIBLE ) {
         ...
         ARISEN_ASSERT( itr == rbi.end() || itr->blocknum == lib_num + 1, reversible_blocks_exception,
         ...
         ARISEN_ASSERT( head->block_num <= last_block_num, reversible_blocks_exception,
         ...
         if( ritr != rbi.rend()
            ...
            ARISEN_ASSERT( rbitr != rbi.end(), reversible_blocks_exception, "pending head block not found in reversible blocks");
            auto rev_id = rbitr->get_block_id();
            ARISEN_ASSERT( rev_id == pending_head->id,
   ...
   void read_from_snapshot( const snapshot_reader_ptr& snapshot, uint32_t blog_start, uint32_t blog_end ) {
      ...
      snapshot->read_section<block_state>([this, blog_start, blog_end]( auto &section ){
         ...
         ARISEN_ASSERT( blog_start <= (snapshot_head_block + 1) && snapshot_head_block <= blog_end,
                     block_log_exception,
   ...
   void create_native_account( account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            a.set_abi(arisenio_contract_abi(abi_def()));
         }
      });
   ...
   transaction_trace_ptr push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline, uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false ) {
      ...
      ARISEN_ASSERT( itr != idx.end(), unknown_transaction_exception, "unknown transaction" );
   ...
   { try {
      ...
      ARISEN_ASSERT( gtrx.delay_until <= self.pending_block_time(), transaction_exception, "this transaction isn't ready",
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
      ...
      ARISEN_ASSERT( net_usage_words*8 == net_usage, transaction_exception, "net_usage is not divisible by 8" );
   ...
   transaction_trace_ptr push_transaction( const transaction_metadata_ptr& trx,
                                           ...
                                           bool explicit_billed_cpu_time = false )
   {
      ARISEN_ASSERT(deadline != fc::time_point(), transaction_exception, "deadline cannot be uninitialized");
      ...
      try {
         ...
         if( !explicit_billed_cpu_time ) {
            fc::microseconds already_consumed_time( ARISEN_PERCENT(sig_cpu_usage.count(), conf.sig_cpu_bill_pct) );
   ...
   {
      ARISEN_ASSERT( !pending, block_validate_exception, "pending block already exists" );
      ...
      if (!self.skip_db_sessions(s)) {
         ARISEN_ASSERT( db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
      ...
      if ( read_mode == db_read_mode::SPECULATIVE || pending->_block_status != controller::block_status::incomplete )
      {
         ...
         if( new_protocol_feature_activations.size() > 0 ) {
            ...
            for( const auto& feature_digest : new_protocol_feature_activations ) {
               ...
               if( res.second ) {
                  ...
                  ARISEN_ASSERT( !f.preactivation_required, protocol_feature_exception,
                              "attempted to activate protocol feature without prior required preactivation: ${digest}",
                              ("digest", feature_digest)
                  );
               } else {
                  ARISEN_ASSERT( !res.first->second, block_validate_exception,
         ...
         ARISEN_ASSERT( handled_all_preactivated_features, block_validate_exception,
         ...
         )
         {
            ...
            ARISEN_ASSERT( gpo.proposed_schedule.version == pbhs.active_schedule_version + 1,
   ...
   void finalize_block()
   {
      ARISEN_ASSERT( pending, block_validate_exception, "it is not valid to finalize when there is no pending block");
      ARISEN_ASSERT( pending->_block_stage.contains<building_block>(), block_validate_exception, "already called finalize_block");
      ...
      uint64_t CPU_TARGET = ARISEN_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
      resource_limits.set_block_parameters(
         ...
         {ARISEN_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, config::maximum_elastic_resource_multiplier, {99, 100}, {1000, 999}}
      );
   ...
   void commit_block( bool add_to_fork_db ) {
      ...
      try {
         ARISEN_ASSERT( pending->_block_stage.contains<completed_block>(), block_validate_exception,
         ...
         if( add_to_fork_db ) {
            ...
            head = fork_db.head();
            ARISEN_ASSERT( bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
         }
      ...
      for( auto itr = new_protocol_features.begin(); itr != new_protocol_features.end(); ++itr ) {
         ...
         switch( status ) {
            case protocol_feature_set::recognized_t::unrecognized:
               ARISEN_THROW( protocol_feature_exception,
            ...
            break;
            default:
               ARISEN_THROW( protocol_feature_exception, "unexpected recognized_t status" );
            break;
         }

         ARISEN_ASSERT( currently_activated_protocol_features.find( f ) == currently_activated_protocol_features.end(),
         ...
         ARISEN_ASSERT( pfs.validate_dependencies( f, dependency_checker ), protocol_feature_exception,
   ...
   { try {
      try {
         ...
         ARISEN_ASSERT( b->block_extensions.size() == 0, block_validate_exception, "no supported block extensions" );
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            if( receipt.trx.contains<packed_transaction>() ) {
               ...
               ARISEN_ASSERT( false, block_validate_exception, "encountered unexpected receipt type" );
            }
         ...
         ARISEN_ASSERT( producer_block_id == ab._id, block_validate_exception, "Block ID does not match",
   ...
   std::future<block_state_ptr> create_block_state_future( const signed_block_ptr& b ) {
      ARISEN_ASSERT( b, block_validate_exception, "null block" );

      auto id = b->id();

      // no reason for a block_state if fork_db already knows about block
      auto existing = fork_db.get_block( id );
      ARISEN_ASSERT( !existing, fork_database_exception, "we already know about this block: ${id}", ("id", id) );

      auto prev = fork_db.get_block_header( b->previous );
      ARISEN_ASSERT( prev, unlinkable_block_exception,
      ...
      return async_thread_pool( thread_pool.get_executor(), [b, prev, control=this]() {
         ...
         ARISEN_ASSERT( b->transaction_mroot == trx_mroot, block_validate_exception,
   ...
   void push_block( std::future<block_state_ptr>& block_state_future ) {
      ...
      ARISEN_ASSERT(!pending, block_validate_exception, "it is not valid to push a block when there is a pending block");
   ...
   void replay_push_block( const signed_block_ptr& b, controller::block_status s ) {
      ...
      try {
         ARISEN_ASSERT( b, block_validate_exception, "trying to push empty block" );
         ARISEN_ASSERT( (s == controller::block_status::irreversible || s == controller::block_status::validated),
         ...
         if( s == controller::block_status::irreversible ) {
            ...
            ARISEN_ASSERT( read_mode != db_read_mode::IRREVERSIBLE, block_validate_exception,
   ...
   void maybe_switch_forks( const block_state_ptr& new_head, controller::block_status s ) {
      ...
      if( new_head->header.previous == head->id ) {
         ...
         if( branches.second.size() > 0 ) {
            ...
            ARISEN_ASSERT( self.head_block_id() == branches.second.back()->header.previous, fork_database_exception,
   ...
   void check_actor_list( const flat_set<account_name>& actors )const {
      ...
      if( conf.actor_whitelist.size() > 0 ) {
         ...
         ARISEN_ASSERT( is_subset,  actor_whitelist_exception,
         ...
         ARISEN_ASSERT( !intersects, actor_blacklist_exception,
   ...
   void check_contract_list( account_name code )const {
      if( conf.contract_whitelist.size() > 0 ) {
         ARISEN_ASSERT( conf.contract_whitelist.find( code ) != conf.contract_whitelist.end(),
                     contract_whitelist_exception,
                     "account '${code}' is not on the contract whitelist", ("code", code)
                   );
      } else if( conf.contract_blacklist.size() > 0 ) {
         ARISEN_ASSERT( conf.contract_blacklist.find( code ) == conf.contract_blacklist.end(),
   ...
   void check_action_list( account_name code, action_name action )const {
      if( conf.action_blacklist.size() > 0 ) {
         ARISEN_ASSERT( conf.action_blacklist.find( std::make_pair(code, action) ) == conf.action_blacklist.end(),
   ...
   void check_key_list( const public_key_type& key )const {
      if( conf.key_blacklist.size() > 0 ) {
         ARISEN_ASSERT( conf.key_blacklist.find( key ) == conf.key_blacklist.end(),
   ...
   void validate_tapos( const transaction& trx )const {
      ...
      ARISEN_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
...
void controller::preactivate_feature( const digest_type& feature_digest ) {
   ...
   switch( status ) {
      case protocol_feature_set::recognized_t::unrecognized:
         if( is_producing_block() ) {
            ARISEN_THROW( subjective_block_production_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         } else {
            ARISEN_THROW( protocol_feature_bad_block_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         }
      break;
      case protocol_feature_set::recognized_t::disabled:
      ...
      default:
         if( is_producing_block() ) {
            ARISEN_THROW( subjective_block_production_exception, "unexpected recognized_t status" );
         } else {
            ARISEN_THROW( protocol_feature_bad_block_exception, "unexpected recognized_t status" );
         }
      break;
   }
   ...
   ARISEN_ASSERT( !is_protocol_feature_activated( feature_digest ),
               protocol_feature_exception,
   ...
   ARISEN_ASSERT( std::find( pso.preactivated_protocol_features.begin(),
   ...
   ARISEN_ASSERT( pfs.validate_dependencies( feature_digest, dependency_checker ),
               protocol_feature_exception,
...
void controller::start_block( block_timestamp_type when, uint16_t confirm_block_count )
{
   ...
   ARISEN_ASSERT( !my->pending, block_validate_exception, "pending block already exists" );
...
transaction_trace_ptr controller::push_transaction( const transaction_metadata_ptr& trx, fc::time_point deadline, uint32_t billed_cpu_time_us ) {
   validate_db_available_size();
   ARISEN_ASSERT( !in_immutable_mode(), transaction_type_exception, "push transaction not allowed in read-only mode" );
   ARISEN_ASSERT( trx && !trx->implicit && !trx->scheduled, transaction_type_exception, "Implicit/Scheduled transaction not allowed" );
...
{
   ARISEN_ASSERT( !in_immutable_mode(), transaction_type_exception, "push scheduled transaction not allowed in read-only mode" );
   validate_db_available_size();
...
void controller::set_action_blacklist( const flat_set< pair<account_name, action_name> >& new_action_blacklist ) {
   for (auto& act: new_action_blacklist) {
      ARISEN_ASSERT(act.first != account_name(), name_type_exception, "Action blacklist - contract name should not be empty");
      ARISEN_ASSERT(act.second != action_name(), action_type_exception, "Action blacklist - action name should not be empty");
   }
...
time_point controller::pending_block_time()const {
   ARISEN_ASSERT( my->pending, block_validate_exception, "no pending block" );
...
block_id_type controller::last_irreversible_block_id() const {
   ...
   ARISEN_ASSERT( BOOST_LIKELY( signed_blk != nullptr ), unknown_block_exception,
...
void controller::write_snapshot( const snapshot_writer_ptr& snapshot ) const {
   ARISEN_ASSERT( !my->pending, block_validate_exception, "cannot take a consistent snapshot with a pending block" );
...
unapplied_transactions_type& controller::get_unapplied_transactions() {
   if ( my->read_mode != db_read_mode::SPECULATIVE ) {
      ARISEN_ASSERT( my->unapplied_transactions.empty(), transaction_exception,
...
void controller::validate_expiration( const transaction& trx )const { try {
   ...
   ARISEN_ASSERT( time_point(trx.expiration) >= pending_block_time(),
               expired_tx_exception,
               "transaction has expired, "
               "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
               ("trx.expiration",trx.expiration)("pending_block_time",pending_block_time()));
   ARISEN_ASSERT( time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
...
void controller::validate_db_available_size() const {
   ...
   ARISEN_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::validate_reversible_available_size() const {
   ...
   ARISEN_ASSERT(free >= guard, reversible_guard_exception, "reversible free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::set_greylist_limit( uint32_t limit ) {
   ARISEN_ASSERT( 0 < limit && limit <= chain::config::maximum_elastic_resource_multiplier,
               misc_exception,
