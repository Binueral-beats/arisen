...
*/
   ...
   void log_irreversible() {
      ...
      if( log_head ) {
         RSN_ASSERT( root_id == log_head->id(), fork_database_exception, "fork database root does not match block log head" );
      } else {
      ...
      if( snapshot ) {
         ...
         if( db.revision() < 1 || !fork_db.head() ) {
            if( fork_db.head() ) {
               ...
               RSN_ASSERT( db.revision() < 1, database_exception,
   ...
   { try {
      try {
         ...
         RSN_ASSERT( b->block_extensions.size() == 0, block_validate_exception, "no supported block extensions" );
...
void controller::preactivate_feature( const digest_type& feature_digest ) {
   ...
   // Even if they don't recognize the feature, the producer could change their nodrsn code to treat it like an
...
unapplied_transactions_type& controller::get_unapplied_transactions() {
   if ( my->read_mode != db_read_mode::SPECULATIVE ) {
      RSN_ASSERT( my->unapplied_transactions.empty(), transaction_exception,
