...
#include <arisenio/chain/merkle.hpp>
...
namespace arisenio { namespace chain {
   ...
   vector<block_header_extensions> block_header::validate_and_extract_header_extensions()const {
      ...
      for( size_t i = 0; i < header_extensions.size(); ++i ) {
         ...
         ARISEN_ASSERT( id >= id_type_lower_bound, invalid_block_header_extension,
         ...
         ARISEN_ASSERT( match, invalid_block_header_extension,
                     "Block header extension with id type ${id} is not supported",
                     ("id", id)
         );

         if( match->enforce_unique ) {
            ARISEN_ASSERT( i == 0 || id > id_type_lower_bound, invalid_block_header_extension,
