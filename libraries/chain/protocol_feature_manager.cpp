...
Removes the restriction on arisenio::linkauth for non-native actions named one of the five special action names:
...
Forward arisenio::setcode actions to the WebAssembly code deployed on the arisenio account.
*/
            {}
         } )
...
*/
   ...
   const char* builtin_protocol_feature_codename( builtin_protocol_feature_t codename ) {
      ...
      ARISEN_ASSERT( itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
   ...
   ,_type( feature_type )
   {
      switch( feature_type ) {
         ...
         default:
         {
            ARISEN_THROW( protocol_feature_validation_exception,
   ...
   {
      ARISEN_ASSERT( builtin_feature, protocol_feature_exception, "not a builtin protocol feature" );
   ...
   const protocol_feature& protocol_feature_set::get_protocol_feature( const digest_type& feature_digest )const {
      ...
      ARISEN_ASSERT( itr != _recognized_protocol_features.end(), protocol_feature_exception,
   ...
   const protocol_feature& protocol_feature_set::add_feature( const builtin_protocol_feature& f ) {
      ...
      ARISEN_ASSERT( builtin_itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
      ...
      if( indx < _recognized_builtin_protocol_features.size() ) {
         ARISEN_ASSERT( _recognized_builtin_protocol_features[indx] == _recognized_protocol_features.end(),
      ...
      if( expected_builtin_dependencies.size() > satisfied_builtin_dependencies.size() ) {
         ...
         for( const auto& builtin_codename : missing_builtins ) {
            ...
            ARISEN_ASSERT( itr != builtin_protocol_feature_codenames.end(),
                        protocol_feature_exception,
                        "Unexpected error"
            );
      ...
      ARISEN_ASSERT( res.second, protocol_feature_exception,
   ...
   void protocol_feature_manager::init( chainbase::database& db ) {
      ARISEN_ASSERT( !is_initialized(), protocol_feature_exception, "cannot initialize protocol_feature_manager twice" );
   ...
   const protocol_feature* protocol_feature_manager::const_iterator::get_pointer()const {
      //ARISEN_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot dereference singular iterator" );
      //ARISEN_ASSERT( _index != end_index, protocol_feature_iterator_exception, "cannot dereference end iterator" );
   ...
   uint32_t protocol_feature_manager::const_iterator::activation_ordinal()const {
      ...
      ARISEN_ASSERT( _index != end_index,
                   protocol_feature_iterator_exception,
   ...
   protocol_feature_manager::const_iterator& protocol_feature_manager::const_iterator::operator++() {
      ARISEN_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot increment singular iterator" );
      ARISEN_ASSERT( _index != end_index, protocol_feature_iterator_exception, "cannot increment end iterator" );
   ...
   protocol_feature_manager::const_iterator& protocol_feature_manager::const_iterator::operator--() {
      ARISEN_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot decrement singular iterator" );
      if( _index == end_index ) {
         ARISEN_ASSERT( _pfm->_activated_protocol_features.size() > 0,
                     protocol_feature_iterator_exception,
   ...
   {
      ARISEN_ASSERT( is_initialized(), protocol_feature_exception, "protocol_feature_manager is not yet initialized" );

      auto itr = _protocol_feature_set.find( feature_digest );

      ARISEN_ASSERT( itr != _protocol_feature_set.end(), protocol_feature_exception,
                  "unrecognized protocol feature digest: ${digest}", ("digest", feature_digest) );

      if( _activated_protocol_features.size() > 0 ) {
         const auto& last = _activated_protocol_features.back();
         ARISEN_ASSERT( last.activation_block_num <= current_block_num,
                     protocol_feature_exception,
      ...
      ARISEN_ASSERT( itr->builtin_feature,
                  protocol_feature_exception,
      ...
      ARISEN_ASSERT( indx < _builtin_protocol_features.size(), protocol_feature_exception,
      ...
      ARISEN_ASSERT( _builtin_protocol_features[indx].activation_block_num == builtin_protocol_feature_entry::not_active,
                  protocol_feature_exception,
