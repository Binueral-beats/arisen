...
namespace arisen { namespace chain {
                                                       ...
                                                       const vector<digest_type>& )>& validator )
   {
      ...
      if( fc::exists( fork_db_dat ) ) {
         try {
            ...
            ARISEN_ASSERT( totem == magic_number, fork_database_exception,
            ...
            ARISEN_ASSERT( version >= min_supported_version && version <= max_supported_version,
            ...
            if( my->root->id == head_id ) {
               ...
               ARISEN_ASSERT( my->head, fork_database_exception,
            ...
            if( candidate == my->index.get<by_lib_block_num>().end() || !(*candidate)->is_valid() ) {
               ARISEN_ASSERT( my->head->id == my->root->id, fork_database_exception,
                           "head not set to root despite no better option available; '${filename}' is likely corrupted",
                           ("filename", fork_db_dat.generic_string()) );
            } else {
               ARISEN_ASSERT( !first_preferred( **candidate, *my->head ), fork_database_exception,
   ...
   void fork_database::advance_root( const block_id_type& id ) {
      ARISEN_ASSERT( my->root, fork_database_exception, "root not yet set" );

      auto new_root = get_block( id );
      ARISEN_ASSERT( new_root, fork_database_exception,
                  "cannot advance root to a block that does not exist in the fork database" );
      ARISEN_ASSERT( new_root->is_valid(), fork_database_exception,
      ...
      for( auto b = new_root; b; ) {
         ...
         ARISEN_ASSERT( b || blocks_to_remove.back() == my->root->id, fork_database_exception, "invariant violation: orphaned branch was present in forked database" );
      }
   ...
   void fork_database_impl::add( const block_state_ptr& n,
                                 ...
                                 const std::function<void( block_timestamp_type,
                                                           ...
                                                           const vector<digest_type>& )>& validator )
   {
      ARISEN_ASSERT( root, fork_database_exception, "root not yet set" );
      ARISEN_ASSERT( n, fork_database_exception, "attempt to add null block state" );

      auto prev_bh = self.get_block_header( n->header.previous );

      ARISEN_ASSERT( prev_bh, unlinkable_block_exception,
      ...
      if( validate ) {
         ...
         } ARISEN_RETHROW_EXCEPTIONS( fork_database_exception, "serialized fork database is incompatible with configured protocol features"  )
      }

      auto inserted = index.insert(n);
      if( !inserted.second ) {
         if( ignore_duplicate ) return;
         ARISEN_THROW( fork_database_exception, "duplicate block added", ("id", n->id) );
      }
   ...
   pair< branch_type, branch_type >  fork_database::fetch_branch_from( const block_id_type& first,
      ...
      ARISEN_ASSERT(first_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", first));
      ARISEN_ASSERT(second_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", second));
      ...
      while( first_branch->block_num > second_branch->block_num )
      {
         ...
         ARISEN_ASSERT( first_branch, fork_db_block_not_found,
      ...
      while( second_branch->block_num > first_branch->block_num )
      {
         ...
         ARISEN_ASSERT( second_branch, fork_db_block_not_found,
                     "block ${id} does not exist",
                     ("id", prev)
         );
      }
   ...
   void fork_database::remove( const block_id_type& id ) {
      ...
      for( uint32_t i = 0; i < remove_queue.size(); ++i ) {
         ARISEN_ASSERT( remove_queue[i] != head_id, fork_database_exception,
   ...
   void fork_database::mark_valid( const block_state_ptr& h ) {
      ...
      ARISEN_ASSERT( itr != by_id_idx.end(), fork_database_exception,
