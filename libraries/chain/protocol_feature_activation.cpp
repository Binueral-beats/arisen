...
#include <arisenio/chain/protocol_feature_activation.hpp>
...
namespace arisenio { namespace chain {
   ...
   void protocol_feature_activation::reflector_init() {
      ...
      ARISEN_ASSERT( protocol_features.size() > 0, ill_formed_protocol_feature_activation,
      ...
      for( const auto& d : protocol_features ) {
         auto res = s.insert( d );
         ARISEN_ASSERT( res.second, ill_formed_protocol_feature_activation,
   ...
   )
   {
      ...
      ARISEN_ASSERT( !enforce_disjoint || protocol_features.size() == expected_size,
