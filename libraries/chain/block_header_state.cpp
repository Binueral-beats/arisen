#include <arisenio/chain/block_header_state.hpp>
...
namespace arisenio { namespace chain {
                                                         ...
                                                         uint16_t num_prev_blocks_to_confirm )const
   {
      ...
      if( when != block_timestamp_type() ) {
        ARISEN_ASSERT( when > header.timestamp, block_validate_exception, "next block must be in the future" );
      } else {
      ...
      if( itr != producer_to_last_produced.end() ) {
        ARISEN_ASSERT( itr->second < (block_num+1) - num_prev_blocks_to_confirm, producer_double_confirm,
   ...
   {
      ARISEN_ASSERT( h.timestamp == timestamp, block_validate_exception, "timestamp mismatch" );
      ARISEN_ASSERT( h.previous == previous, unlinkable_block_exception, "previous mismatch" );
      ARISEN_ASSERT( h.confirmed == confirmed, block_validate_exception, "confirmed mismatch" );
      ARISEN_ASSERT( h.producer == producer, wrong_producer, "wrong producer specified" );
      ARISEN_ASSERT( h.schedule_version == active_schedule_version, producer_schedule_exception, "schedule_version in signed block is corrupted" );

      if( h.new_producers ) {
         ARISEN_ASSERT( !was_pending_promoted, producer_schedule_exception, "cannot set pending producer schedule in the same block in which pending was promoted to active" );
         ARISEN_ASSERT( h.new_producers->version == active_schedule.version + 1, producer_schedule_exception, "wrong producer schedule version specified" );
         ARISEN_ASSERT( prev_pending_schedule.schedule.producers.size() == 0, producer_schedule_exception,
   ...
   void block_header_state::sign( const std::function<signature_type(const digest_type&)>& signer ) {
      ...
      ARISEN_ASSERT( block_signing_key == fc::crypto::public_key( header.producer_signature, d ),
   ...
   void block_header_state::verify_signee( const public_key_type& signee )const {
      ARISEN_ASSERT( block_signing_key == signee, wrong_signing_key,
