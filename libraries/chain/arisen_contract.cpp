...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::arisenio_code_name ) // virtual arisenio.code permission does not really exist but is allowed
         continue;

      try {
         context.control.get_authorization_manager().get_permission({a.permission.actor, a.permission.permission});
      } catch( const permission_query_exception& ) {
         ARISEN_THROW( action_validate_exception,
 ...
 */
void apply_arisenio_newaccount(apply_context& context) {
   auto create = context.get_action().data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
//   context.require_write_lock( config::arisenio_auth_scope );
   auto& authorization = context.control.get_mutable_authorization_manager();

   ARISEN_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   ARISEN_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");

   auto& db = context.db;

   auto name_str = name(create.name).to_string();

   ARISEN_ASSERT( !create.name.empty(), action_validate_exception, "account name cannot be empty" );
   ARISEN_ASSERT( name_str.size() <= 12, action_validate_exception, "account names can only be 12 chars long" );

   // Check if the creator is privileged
   const auto &creator = db.get<account_metadata_object, by_name>(create.creator);
   if( !creator.is_privileged() ) {
      ARISEN_ASSERT( name_str.find( "arisenio." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'arisenio.'" );
   }

   auto existing_account = db.find<account_object, by_name>(create.name);
   ARISEN_ASSERT(existing_account == nullptr, account_name_exists_exception,
...
void apply_arisenio_setcode(apply_context& context) {
   ...
   ARISEN_ASSERT( act.vmtype == 0, invalid_contract_vm_type, "code should be 0" );
   ARISEN_ASSERT( act.vmversion == 0, invalid_contract_vm_version, "version should be 0" );
   ...
   ARISEN_ASSERT( code_size > 0 || existing_code, set_exact_code, "contract is already cleared" );
   ...
   if( existing_code ) {
      ...
      ARISEN_ASSERT( old_code_entry.code_hash != code_hash, set_exact_code,
...
void apply_arisenio_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_arisenio_updateauth(apply_context& context) {
   ...
   ARISEN_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   ARISEN_ASSERT( update.permission.to_string().find( "arisenio." ) != 0, action_validate_exception,
               "Permission names that start with 'arisenio.' are reserved" );
   ARISEN_ASSERT(update.permission != update.parent, action_validate_exception, "Cannot set an authority as its own parent");
   db.get<account_object, by_name>(update.account);
   ARISEN_ASSERT(validate(update.auth), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.auth));
   if( update.permission == config::active_name )
      ARISEN_ASSERT(update.parent == config::owner_name, action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == config::owner_name)
      ARISEN_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   else
      ARISEN_ASSERT(!update.parent.empty(), action_validate_exception, "Only owner permission can have empty parent" );

   if( update.auth.waits.size() > 0 ) {
      auto max_delay = context.control.get_global_properties().configuration.max_transaction_delay;
      ARISEN_ASSERT( update.auth.waits.back().wait_sec <= max_delay, action_validate_exception,
   ...
   if( permission ) {
      ARISEN_ASSERT(parent_id == permission->parent, action_validate_exception,
...
void apply_arisenio_deleteauth(apply_context& context) {
//   context.require_write_lock( config::arisenio_auth_scope );
   ...
   ARISEN_ASSERT(remove.permission != config::active_name, action_validate_exception, "Cannot delete active authority");
   ARISEN_ASSERT(remove.permission != config::owner_name, action_validate_exception, "Cannot delete owner authority");
   ...
   { // Check for links to this permission
      ...
      ARISEN_ASSERT(range.first == range.second, action_validate_exception,
...
void apply_arisenio_linkauth(apply_context& context) {
//   context.require_write_lock( config::arisenio_auth_scope );

   auto requirement = context.get_action().data_as<linkauth>();
   try {
      ARISEN_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
      ...
      ARISEN_ASSERT(account != nullptr, account_query_exception,
      ...
      ARISEN_ASSERT(code != nullptr, account_query_exception,
                 "Failed to retrieve code for account: ${account}", ("account", requirement.code));
      if( requirement.requirement != config::arisenio_any_name ) {
         ...
         ARISEN_ASSERT(permission != nullptr, permission_query_exception,
      ...
      if( link ) {
         ARISEN_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
...
void apply_arisenio_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::arisenio_auth_scope );
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   ARISEN_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
   context.add_ram_usage(
      link->account,
      -(int64_t)(config::billable_size_v<permission_link_object>)
   );
...
void apply_arisenio_canceldelay(apply_context& context) {
