...
void apply_context::exec_one()
{
         ...
         if( !privileged && control.is_builtin_activated( builtin_protocol_feature_t::ram_restrictions ) ) {
            ...
            for( auto itr = _account_ram_deltas.begin(); itr != end; ++itr, ++counter ) {
               ...
               if( itr->delta > 0 && itr->account != receiver ) {
                  ARISEN_ASSERT( not_in_notify_context, unauthorized_ram_usage_increase,
                              "unprivileged contract cannot increase RAM usage of another account within a notify context: ${account}",
                              ("account", itr->account)
                  );
                  ARISEN_ASSERT( has_authorization( itr->account ), unauthorized_ram_usage_increase,
   ...
   //    * and, the *receiver_account object itself cannot be removed because accounts cannot be deleted in ARISENIO.
...
void apply_context::exec()
{
   ...
   if( _cfa_inline_actions.size() > 0 || _inline_actions.size() > 0 ) {
      ARISEN_ASSERT( recurse_depth < control.get_global_properties().configuration.max_inline_action_depth,
...
void apply_context::require_authorization( const account_name& account ) {
   ...
   ARISEN_ASSERT( false, missing_auth_exception, "missing authority of ${account}", ("account",account));
}
...
void apply_context::require_authorization(const account_name& account,
  ...
  ARISEN_ASSERT( false, missing_auth_exception, "missing authority of ${account}/${permission}",
...
void apply_context::execute_inline( action&& a ) {
   ...
   ARISEN_ASSERT( code != nullptr, action_validate_exception,
   ...
   for( const auto& auth : a.authorization ) {
      ...
      ARISEN_ASSERT( actor != nullptr, action_validate_exception,
                  "inline action's authorizing actor ${account} does not exist", ("account", auth.actor) );
      ARISEN_ASSERT( control.get_authorization_manager().find_permission(auth) != nullptr, action_validate_exception,
      ...
      try {
         control.get_authorization_manager()
                .check_authorization( {a},
                                      {},
                                      {{receiver, config::arisenio_code_name}},
         ...
         if( disallow_send_to_self_bypass || !send_to_self ) {
            ...
            ARISEN_THROW(subjective_block_production_exception, "Unexpected exception occurred validating inline action sent to self");
         }
      }
   }
...
void apply_context::execute_context_free_inline( action&& a ) {
   ...
   ARISEN_ASSERT( a.authorization.size() == 0, action_validate_exception,
...
void apply_context::schedule_deferred_transaction( const uint128_t& sender_id, account_name payer, transaction&& trx, bool replace_existing ) {
   ARISEN_ASSERT( trx.context_free_actions.size() == 0, cfa_inside_generated_tx, "context free actions are not currently allowed in generated transactions" );
   ...
   if( control.is_builtin_activated( builtin_protocol_feature_t::no_duplicate_deferred_id ) ) {
      ...
      if( exts.size() > 0 ) {
         ARISEN_ASSERT( exts.size() == 1, invalid_transaction_extension,
         ...
         ARISEN_ASSERT( context.sender == receiver, ill_formed_deferred_transaction_generation_context,
                     "deferred transaction generaction context contains mismatching sender",
                     ("expected", receiver)("actual", context.sender)
         );
         ARISEN_ASSERT( context.sender_id == sender_id, ill_formed_deferred_transaction_generation_context,
                     "deferred transaction generaction context contains mismatching sender_id",
                     ("expected", sender_id)("actual", context.sender_id)
         );
         ARISEN_ASSERT( context.sender_trx_id == trx_context.id, ill_formed_deferred_transaction_generation_context,
   ...
   if( !control.skip_auth_check() && !privileged ) { // Do not need to check authorization if replayng irreversible block or if contract is privileged
      if( payer != receiver ) {
         if( ram_restrictions_activated ) {
            ARISEN_ASSERT( receiver == act->account, action_validate_exception,
                        "cannot bill RAM usage of deferred transactions to another account within notify context"
            );
            ARISEN_ASSERT( has_authorization( payer ), action_validate_exception,
      ...
      try {
         ...
         if( disallow_send_to_self_bypass || !is_sending_only_to_self(receiver) ) {
            ...
            ARISEN_THROW(subjective_block_production_exception, "Unexpected exception occurred validating sent deferred transaction consisting only of actions to self");
         }
      }
   }
   ...
   if ( auto ptr = db.find<generated_transaction_object,by_sender_id>(boost::make_tuple(receiver, sender_id)) ) {
      ARISEN_ASSERT( replace_existing, deferred_tx_duplicate, "deferred transaction with the same sender_id and payer already exists" );

      bool replace_deferred_activated = control.is_builtin_activated(builtin_protocol_feature_t::replace_deferred);

      ARISEN_ASSERT( replace_deferred_activated || !control.is_producing_block()
   ...
   ARISEN_ASSERT( ram_restrictions_activated
...
void apply_context::update_db_usage( const account_name& payer, int64_t delta ) {
   if( delta > 0 ) {
      ...
      {
         ARISEN_ASSERT( control.is_ram_billing_in_notify_allowed() || (receiver == act->account),
...
int apply_context::get_action( uint32_t type, uint32_t index, char* buffer, size_t buffer_size )const
{
   ...
   ARISEN_ASSERT(act_ptr, action_not_found_exception, "action is not found" );
...
//   require_write_lock( scope );
   ...
   ARISEN_ASSERT( payer != account_name(), invalid_table_payer, "must specify a valid account to pay for new record" );
...
void apply_context::db_update_i64( int iterator, account_name payer, const char* buffer, size_t buffer_size ) {
   ...
   ARISEN_ASSERT( table_obj.code == receiver, table_access_violation, "db access violation" );
...
int apply_context::db_previous_i64( int iterator, uint64_t& primary ) {
   ...
   if( iterator < -1 ) // is end iterator
   {
      ...
      ARISEN_ASSERT( tab, invalid_table_iterator, "not a valid end iterator" );
