...
bool is_access_violation(fc::unhandled_exception const & e) {
   try {
    ...
    catch (const arisenio::chain::wasm_execution_error& e) {
       return true;
    } catch (...) {
...
bool is_wasm_execution_error(arisenio::chain::wasm_execution_error const& e) {return true;}
...
BOOST_FIXTURE_TEST_CASE(action_receipt_tests, TESTER) { try {
   ...
   set_code( N(test), contracts::arisenio_bios_wasm() );
   set_abi( N(test), contracts::arisenio_bios_abi().data() );
	set_code( N(test), contracts::payloadless_wasm() );
...
BOOST_FIXTURE_TEST_CASE(action_tests, TESTER) { try {
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "assert_false", {} ),
                          arisenio_assert_message_exception, arisenio_assert_message_is("test_action::assert_false") );
   ...
   BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION( *this, "test_action", "read_action_to_0", raw_bytes), arisenio::chain::wasm_execution_error,
         [](const arisenio::chain::wasm_execution_error& e) {
   ...
   // test read_action_to_64k
   raw_bytes.resize(3);
	BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION( *this, "test_action", "read_action_to_64k", raw_bytes ), arisenio::chain::wasm_execution_error,
         [](const arisenio::chain::wasm_execution_error& e) {
            return expect_assert_message(e, "access violation");
         }
      );
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "test_current_time", fc::raw::pack(now) ),
                          arisenio_assert_message_exception, arisenio_assert_message_is("tmp == current_time()")     );
...
BOOST_FIXTURE_TEST_CASE(cf_action_tests, TESTER) { try {
      ...
      BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_transaction", "send_cf_action_fail", {} ),
                             arisenio_assert_message_exception,
                             arisenio_assert_message_is("context free actions cannot have authorizations") );
...
BOOST_FIXTURE_TEST_CASE(transaction_tests, TESTER) { try {
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION(*this, "test_transaction", "send_action_inline_fail", {}),
                          arisenio_assert_message_exception,
                          arisenio_assert_message_is("test_action::assert_false")                          );
   ...
   BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_transaction", "send_action_recurse", {}), arisenio::chain::transaction_exception,
         [](const arisenio::chain::transaction_exception& e) {
...
BOOST_FIXTURE_TEST_CASE(deferred_transaction_tests, TESTER) { try {
...

   {
      ...
      auth.accounts.push_back( permission_level_weight{{N(testapi), config::arisenio_code_name}, 1} );
      ...
      // fail because testapi@additional permission is not unilaterally satisfied by testapi@arisenio.code.
      dtt_action dtt_act3;
...
BOOST_AUTO_TEST_CASE(more_deferred_transaction_tests) { try {
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      arisenio_assert_message_exception,
      arisenio_assert_message_is("fail")
   );
...
BOOST_FIXTURE_TEST_CASE(multi_index_tests, TESTER) { try {
   ...
   auto check_failure = [this]( action_name a, const char* expected_error_msg ) {
      BOOST_CHECK_EXCEPTION(  push_action( N(testapi), a, N(testapi), {} ),
                              arisenio_assert_message_exception,
                              arisenio_assert_message_is( expected_error_msg )
      );
   };
...
BOOST_FIXTURE_TEST_CASE(permission_tests, TESTER) { try {
   ...
   CALL_TEST_FUNCTION( *this, "test_permission", "check_authorization",
      fc::raw::pack( check_auth {
         ...
         .permission = N(active),
         .pubkeys    = {
            public_key_type(string("ARISEN7GfRtyDWWgxV88a5TRaYY59XmHptyfjsFmHHfioGNJtPjpSmGX"))
         }
      })
   );
...
/*************************************************************************************
 * arisenio_assert_code_tests test cases
 *************************************************************************************/
BOOST_FIXTURE_TEST_CASE(arisenio_assert_code_tests, TESTER) { try {
   produce_block();
   create_account( N(testapi) );
   produce_block();
   set_code(N(testapi), contracts::test_api_wasm() );
   ...
   const char* abi_string = R"=====(
{
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "test_assert_code", fc::raw::pack((uint64_t)42) ),
                          arisenio_assert_code_exception, arisenio_assert_code_is(42)                                        );
