...
#include <arisenio/testing/tester_network.hpp>
...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(arisenio.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(arisenio.token), contracts::arisenio_token_wasm() );
         chain->set_abi(N(arisenio.token), contracts::arisenio_token_abi().data() );
         chain->push_action( N(arisenio.token), N(create), N(arisenio.token), mvo()
              ( "issuer", "arisenio.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(arisenio.token), N(issue), N(arisenio.token), mvo()
              ( "to", "arisenio.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(arisenio.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(arisenio.token), N(alice)};
   test.init();

   test.transfer( N(arisenio.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(arisenio.token), N(bob)};
   test.init();

   test.transfer( N(arisenio.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(arisenio.token) );
   ...
   test.chain->set_code(N(bob), contracts::arisenio_token_wasm() );
   test.chain->set_abi(N(bob), contracts::arisenio_token_abi().data() );

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), contracts::arisenio_token_wasm() );
   test.chain->set_abi(N(charlie), contracts::arisenio_token_abi().data() );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(arisenio.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_arisenio ) { try {
   whitelist_blacklist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code(config::system_account_name, contracts::arisenio_token_wasm() );
   tester1.chain->produce_blocks();
   tester1.shutdown();
...
BOOST_AUTO_TEST_CASE( blacklist_onerror ) { try {
   ...
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("action 'arisenio::onerror' is on the action blacklist")
                        );
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   auto auth = authority(arisenio::testing::base_tester::get_public_key("alice", "active"));
   auth.accounts.push_back( permission_level_weight{{N(alice), config::arisenio_code_name}, 1} );

   tester1.chain->push_action( N(arisenio), N(updateauth), N(alice), mvo()
      ( "account", "alice" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(arisenio::testing::base_tester::get_public_key("bob", "active"));
   auth.accounts.push_back( permission_level_weight{{N(alice), config::arisenio_code_name}, 1} );
   auth.accounts.push_back( permission_level_weight{{N(bob), config::arisenio_code_name}, 1} );

   tester1.chain->push_action( N(arisenio), N(updateauth), N(bob), mvo()
      ( "account", "bob" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(arisenio::testing::base_tester::get_public_key("charlie", "active"));
   auth.accounts.push_back( permission_level_weight{{N(charlie), config::arisenio_code_name}, 1} );

   tester1.chain->push_action( N(arisenio), N(updateauth), N(charlie), mvo()
      ( "account", "charlie" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auto log_trxs = [&](std::tuple<const transaction_trace_ptr&, const signed_transaction&> x) {
      ...
      if( act.account == N(arisenio) && act.name == N(onblock) ) return;
