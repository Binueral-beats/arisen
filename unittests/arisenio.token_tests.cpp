...
class arisenio_token_tester : public tester {
public:

   arisenio_token_tester() {
      produce_blocks( 2 );

      create_accounts( { N(alice), N(bob), N(carol), N(arisenio.token) } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = arisenio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(arisenio.token), acc, N(accounts), symbol_code );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( N(arisenio.token), N(create), mvo()
           ( "issuer", issuer)
...
BOOST_AUTO_TEST_SUITE(arisenio_token_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, arisenio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, arisenio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, arisenio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, arisenio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, arisenio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, arisenio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, arisenio_token_tester ) try {
