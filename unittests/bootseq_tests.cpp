...
public:
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "arisenio_global_state", data, abi_serializer_max_time );
   }
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(arisenio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create arisenio.msig and arisenio.token
        create_accounts({N(arisenio.msig), N(arisenio.token), N(arisenio.ram), N(arisenio.rfee), N(arisenio.stake), N(arisenio.vpay), N(arisenio.bpay), N(arisenio.saving) });
        // Set code for the following accounts:
        //  - arisenio (code: arisenio.bios) (already set by tester constructor)
        //  - arisenio.msig (code: arisenio.msig)
        //  - arisenio.token (code: arisenio.token)
        // set_code_abi(N(arisenio.msig), contracts::arisenio_msig_wasm(), contracts::arisenio_msig_abi().data());//, &arisenio_active_pk);
        // set_code_abi(N(arisenio.token), contracts::arisenio_token_wasm(), contracts::arisenio_token_abi().data()); //, &arisenio_active_pk);

        set_code_abi(N(arisenio.msig),
                     contracts::arisenio_msig_wasm(),
                     contracts::arisenio_msig_abi().data());//, &arisenio_active_pk);
        set_code_abi(N(arisenio.token),
                     contracts::arisenio_token_wasm(),
                     contracts::arisenio_token_abi().data()); //, &arisenio_active_pk);

        // Set privileged for arisenio.msig and arisenio.token
        set_privileged(N(arisenio.msig));
        set_privileged(N(arisenio.token));

        // Verify arisenio.msig and arisenio.token is privileged
        const auto& arisenio_msig_acc = get<account_metadata_object, by_name>(N(arisenio.msig));
        BOOST_TEST(arisenio_msig_acc.is_privileged() == true);
        const auto& arisenio_token_acc = get<account_metadata_object, by_name>(N(arisenio.token));
        BOOST_TEST(arisenio_token_acc.is_privileged() == true);


        // Create RSN tokens in arisenio.token, set its manager as arisenio
        ...
        create_currency(N(arisenio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion RSN tokens to arisenio.system
        issue(N(arisenio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(arisenio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "arisenio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), arisenio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), arisenio_assert_message_exception);
