...
public:
   ...
   ARISEN_system_tester(Lambda setup) {
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("arisen")  + get_balance("arisen.rfee") + get_balance("arisen.stake") + get_balance("arisen.ram") );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& arisenin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",arisenin) );
   }
...
inline uint64_t M( const string& arisen_str ) {
   return core_from_string( arisen_str ).get_amount();
}
