...
BOOST_AUTO_TEST_CASE( activate_preactivate_feature ) try {
   ...
   BOOST_CHECK_EXCEPTION( c.set_code( config::system_account_name, contracts::arisenio_bios_wasm() ),
                          wasm_exception, fc_exception_message_is("env.is_feature_activated unresolveable")
   );

   // But the old bios contract can still be set.
   c.set_code( config::system_account_name, contracts::before_preactivate_arisenio_bios_wasm() );
   c.set_abi( config::system_account_name, contracts::before_preactivate_arisenio_bios_abi().data() );
   ...
   c.set_code( config::system_account_name, contracts::arisenio_bios_wasm() );
   c.set_abi( config::system_account_name, contracts::arisenio_bios_abi().data() );
   ...
   BOOST_CHECK_EXCEPTION( c.push_action( config::system_account_name, N(reqactivated), config::system_account_name,
                           ...
                           arisenio_assert_message_is( "protocol feature is not activated" )
   );
...
BOOST_AUTO_TEST_CASE( no_duplicate_deferred_id_test ) try {
   ...
   // Retire the delayed arisenio::reqauth transaction.
   c.produce_blocks(5);
   BOOST_REQUIRE( trace1 );
   BOOST_REQUIRE_EQUAL(0, index.size());
...
BOOST_AUTO_TEST_CASE( fix_linkauth_restriction ) { try {
   ...
   auto validate_disallow = [&] (const char *code, const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link arisenio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };

   validate_disallow("arisenio", "linkauth");
   validate_disallow("arisenio", "unlinkauth");
   validate_disallow("arisenio", "deleteauth");
   validate_disallow("arisenio", "updateauth");
   validate_disallow("arisenio", "canceldelay");
...
BOOST_AUTO_TEST_CASE( forward_setcode_test ) { try {
   ...
   //   * arisenio::setcode to set code on the arisenio is allowed (unless the rejectall account exists)
   //   * arisenio::newaccount is allowed only if it creates the rejectall account.
   c.set_code( config::system_account_name, contracts::reject_all_wasm() );
   c.produce_block();

   // Before activation, deploying a contract should work since setcode won't be forwarded to the WASM on arisenio.
   c.set_code( tester1_account, contracts::noop_wasm() );

   // Activate FORWARD_SETCODE protocol feature and then return contract on arisenio back to what it was.
   const auto& pfm = c.control->get_protocol_feature_manager();
   ...
   // After activation, deploying a contract causes setcode to be dispatched to the WASM on arisenio,
   ...
   BOOST_REQUIRE_EXCEPTION( c.set_code( tester2_account, contracts::noop_wasm() ),
                            arisenio_assert_message_exception,
                            arisenio_assert_message_is( "rejecting all actions" ) );
   ...
   // It will now not be possible to deploy the reject_all contract to the arisenio account,
   ...
   // already deployed on arisenio.
   c2.create_account( N(rejectall) );
   c2.produce_block();
   // Now all actions dispatched to the arisenio account should be rejected.

   // However, it should still be possible to set the bios contract because the WASM on arisenio is called after the
...
BOOST_AUTO_TEST_CASE( get_sender_test ) { try {
   ...
   BOOST_CHECK_EXCEPTION(  c.push_action( tester1_account, N(sendinline), tester1_account, mutable_variant_object()
                           ...
                           arisenio_assert_message_exception,
                           arisenio_assert_message_is( "sender did not match" ) );
