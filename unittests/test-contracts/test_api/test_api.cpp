...
extern "C" {
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) {
      if( code == "arisenio"_n.value && action == "onerror"_n.value ) {
         auto error = arisenio::onerror::from_current_action();
         arisenio::print("onerror called\n");
      ...
      //unhandled test call
      arisenio_assert( false, "Unknown Test" );
