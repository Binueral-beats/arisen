...
#include <ariseniolib/datastream.hpp>
#include <ariseniolib/db.h>
#include <ariseniolib/arisenio.hpp>
#include <ariseniolib/print.hpp>
#include <ariseniolib/privileged.h>
...
void test_action::read_action_normal() {
   ...
   arisenio_assert( action_data_size() == sizeof(dummy_action), "action_size() == sizeof(dummy_action)" );

   total = read_action_data( buffer, 30 );
   arisenio_assert( total == sizeof(dummy_action) , "read_action(30)" );

   total = read_action_data( buffer, 100 );
   arisenio_assert(total == sizeof(dummy_action) , "read_action(100)" );

   total = read_action_data(buffer, 5);
   arisenio_assert( total == 5 , "read_action(5)" );

   total = read_action_data(buffer, sizeof(dummy_action) );
   arisenio_assert( total == sizeof(dummy_action), "read_action(sizeof(dummy_action))" );

   dummy_action *dummy13 = reinterpret_cast<dummy_action *>(buffer);

   arisenio_assert( dummy13->a == DUMMY_ACTION_DEFAULT_A, "dummy13->a == DUMMY_ACTION_DEFAULT_A" );
   arisenio_assert( dummy13->b == DUMMY_ACTION_DEFAULT_B, "dummy13->b == DUMMY_ACTION_DEFAULT_B" );
   arisenio_assert( dummy13->c == DUMMY_ACTION_DEFAULT_C, "dummy13->c == DUMMY_ACTION_DEFAULT_C" );
}
...
void test_action::test_dummy_action() {
   ...
   arisenio_assert( total > 0, "get_action failed" );
   arisenio::action act = arisenio::get_action( 1, 0 );
   arisenio_assert( act.authorization.back().actor == "testapi"_n, "incorrect permission actor" );
   arisenio_assert( act.authorization.back().permission == "active"_n, "incorrect permission name" );
   arisenio_assert( arisenio::pack_size(act) == static_cast<size_t>(total), "pack_size does not match get_action size" );
   arisenio_assert( act.account == "testapi"_n, "expected testapi account" );
   ...
   if ( dum13.b == 200 ) {
      ...
      arisenio_assert( false, "get_context_free_data() not allowed in non-context free action" );
   } else {
      arisenio_assert( dum13.a == DUMMY_ACTION_DEFAULT_A, "dum13.a == DUMMY_ACTION_DEFAULT_A" );
      arisenio_assert( dum13.b == DUMMY_ACTION_DEFAULT_B, "dum13.b == DUMMY_ACTION_DEFAULT_B" );
      arisenio_assert( dum13.c == DUMMY_ACTION_DEFAULT_C, "dum13.c == DUMMY_ACTION_DEFAULT_C" );
   }
}
...
void test_action::test_cf_action() {
   ...
   arisenio::action act = arisenio::get_action( 0, 0 );
   ...
   if ( cfa.payload == 100 ) {
      ...
      arisenio_assert( size > 0, "size determination failed" );
      std::vector<char> cfd( static_cast<size_t>(size) );
      size = get_context_free_data( cfa.cfd_idx, &cfd[0], static_cast<size_t>(size) );
      arisenio_assert(static_cast<size_t>(size) == cfd.size(), "get_context_free_data failed" );
      uint32_t v = arisenio::unpack<uint32_t>( &cfd[0], cfd.size() );
      arisenio_assert( v == cfa.payload, "invalid value" );
      ...
      arisenio_assert(transaction_size() > 0, "transaction_size failed");
      // verify softfloat api access
      float f1 = 1.0f, f2 = 2.0f;
      float f3 = f1 + f2;
      arisenio_assert( f3 >  2.0f, "Unable to add float.");
      // verify context_free_system_api
      arisenio_assert( true, "verify arisenio_assert can be called" );


   } else if ( cfa.payload == 200 ) {
      // attempt to access non context free api, privileged_api
      is_privileged(act.name.value);
      arisenio_assert( false, "privileged_api should not be allowed" );
   } else if ( cfa.payload == 201 ) {
      // attempt to access non context free api, producer_api
      get_active_producers( nullptr, 0 );
      arisenio_assert( false, "producer_api should not be allowed" );
      ...
      arisenio_assert( false, "db_api should not be allowed" );
      ...
      arisenio::action dum_act;
      dum_act.send();
      arisenio_assert( false, "action send should not be allowed" );
   } else if ( cfa.payload == 206 ) {
      arisenio::require_auth("test"_n);
      arisenio_assert( false, "authorization_api should not be allowed" );
   } else if ( cfa.payload == 207 ) {
      now();
      arisenio_assert( false, "system_api should not be allowed" );
      ...
      arisenio_assert( false, "transaction_api should not be allowed" );
...
void test_action::require_notice( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   if( receiver == "testapi"_n.value ) {
      arisenio::require_recipient( "acc1"_n );
      arisenio::require_recipient( "acc2"_n );
      arisenio::require_recipient( "acc1"_n, "acc2"_n );
      arisenio_assert( false, "Should've failed" );
...
void test_action::require_notice_tests( uint64_t receiver, uint64_t code, uint64_t action ) {
   arisenio::print( "require_notice_tests" );
   if( receiver == "testapi"_n.value ) {
      arisenio::print("require_recipient( \"acc5\"_n )");
      arisenio::require_recipient("acc5"_n);
   } else if( receiver == "acc5"_n.value ) {
      arisenio::print("require_recipient( \"testapi\"_n )");
      arisenio::require_recipient("testapi"_n);
   }
}

void test_action::require_auth() {
   prints("require_auth");
   arisenio::require_auth("acc3"_n);
   arisenio::require_auth("acc4"_n);
}

void test_action::assert_false() {
   arisenio_assert( false, "test_action::assert_false" );
}

void test_action::assert_true() {
   arisenio_assert( true, "test_action::assert_true" );
}
...
void test_action::test_abort() {
   abort();
   arisenio_assert( false, "should've aborted" );
}

void test_action::test_publication_time() {
   uint64_t pub_time = 0;
   uint32_t total = read_action_data( &pub_time, sizeof(uint64_t) );
   arisenio_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)" );
   arisenio_assert( pub_time == publication_time(), "pub_time == publication_time()" );
}
...
void test_action::test_current_receiver( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   arisenio_assert( receiver == cur_rec.value, "the current receiver does not match" );
}
...
void test_action::test_current_time() {
   ...
   arisenio_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)" );
   arisenio_assert( tmp == current_time(), "tmp == current_time()" );
}
...
void test_action::test_assert_code() {
   ...
   arisenio_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)" );
   arisenio_assert_code( false, code );
}
...
void test_action::test_ram_billing_in_notify( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   arisenio_assert( total == sizeof(uint128_t), "total == sizeof(uint128_t)" );

   uint64_t to_notify = tmp >> 64;
   uint64_t payer = tmp & 0xFFFFFFFFFFFFFFFFULL;

   if( code == receiver ) {
      arisenio::require_recipient( name{to_notify} );
   } else {
      arisenio_assert( to_notify == receiver, "notified recipient other than the one specified in to_notify" );
...
void test_action::test_action_ordinal1(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (receiver == "testapi"_n.value) {
      print("exec 1");
      arisenio::require_recipient( "bob"_n ); //-> exec 2 which would then cause execution of 4, 10

      arisenio::action act1({name(_self), "active"_n}, name(_self),
      ...
      if (is_account("fail1"_n)) {
         arisenio_assert(false, "fail at point 1");
      }

      arisenio::action act2({name(_self), "active"_n}, name(_self),
                         name(WASM_TEST_ACTION("test_action", "test_action_ordinal3")),
                         std::tuple<>());
      act2.send(); // -> exec 9

      arisenio::require_recipient( "charlie"_n ); // -> exec 3 which would then cause execution of 11
      ...
      arisenio::require_recipient( "david"_n );  // -> exec 4
      ...
      if (is_account("fail3"_n)) {
         arisenio_assert(false, "fail at point 3");
      }

   } else if (receiver == "david"_n.value) {
      print("exec 4");
   } else {
      arisenio_assert(false, "assert failed at test_action::test_action_ordinal1");
   }
}
void test_action::test_action_ordinal2(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (receiver == "testapi"_n.value) {
      print("exec 5");
      arisenio::require_recipient( "david"_n ); // -> exec 6
      arisenio::require_recipient( "erin"_n ); // -> exec 7
      ...
      arisenio_assert(false, "assert failed at test_action::test_action_ordinal2");
   }
}
...
void test_action::test_action_ordinal3(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (is_account("failnine"_n)) {
      arisenio_assert(false, "fail at point 9");
   }
}
