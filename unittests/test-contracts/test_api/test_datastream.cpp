...
struct testtype {
    static void run( const T &v, const char *errmsg = "" )  {
        char buf[128];
        arisenio::datastream<char *> ds( buf, sizeof(buf) );
        ds << v;
        T v2;
        ds.seekp(0);
        ds >> v2;
        arisenio_assert ( v == v2, errmsg );   
    }
};
...
struct testtype<double> {
   static void run( const double &v, const char *errmsg = "" ) {
      ...
      ds.seekp(0);
      ds >> v2;
      arisenio_assert( std::abs(v - v2) < 1e-20, errmsg );
   }
};
...
struct testtype<float> {
   static void run( const float &v, const char *errmsg = "" ) {
      ...
      ds.seekp(0);
      ds >> v2;
      arisenio_assert( std::abs(v - v2) < float(1e-10), errmsg );
   }
};
