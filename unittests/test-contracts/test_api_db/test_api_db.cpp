...
using namespace arisenio::internal_use_do_not_use;
...
void test_api_db::primary_i64_general()
{
   ...
   // find
   {
      ...
      arisenio_assert( itr_next == itr_next_expected && prim == "allyson"_n.value, "primary_i64_general - db_find_i64"  );
      itr_next = db_next_i64( itr_next, &prim );
      itr_next_expected = db_find_i64( receiver, receiver, table1, "bob"_n.value );
      arisenio_assert( itr_next == itr_next_expected && prim == "bob"_n.value, "primary_i64_general - db_next_i64" );
   }
   ...
   // next
   {
      ...
      arisenio_assert( end_itr < 0, "primary_i64_general - db_next_i64" );
      // prim didn't change
      arisenio_assert( prim == 0, "primary_i64_general - db_next_i64" );
   }
   ...
   // previous
   {
      ...
      arisenio_assert( itr_prev == itr_prev_expected && prim == "bob"_n.value, "primary_i64_general - db_previous_i64" );

      itr_prev = db_previous_i64( itr_prev, &prim );
      itr_prev_expected = db_find_i64( receiver, receiver, table1, "allyson"_n.value );
      arisenio_assert( itr_prev == itr_prev_expected && prim == "allyson"_n.value, "primary_i64_general - db_previous_i64" );

      itr_prev = db_previous_i64( itr_prev, &prim );
      itr_prev_expected = db_find_i64( receiver, receiver, table1, "alice"_n.value );
      arisenio_assert( itr_prev == itr_prev_expected && prim == "alice"_n.value, "primary_i64_general - db_previous_i64" );

      itr_prev = db_previous_i64( itr_prev, &prim );
      arisenio_assert( itr_prev < 0 && prim == "alice"_n.value, "primary_i64_general - db_previous_i64" );
   }

   // remove
   {
      int itr = db_find_i64( receiver, receiver, table1, "alice"_n.value );
      arisenio_assert( itr >= 0, "primary_i64_general - db_find_i64" );
      db_remove_i64( itr );
   ...
   // get
   {
      ...
      arisenio_assert( itr >= 0, "" );
      uint32_t buffer_len = 5;
      char value[50];
      auto len = db_get_i64( itr, value, buffer_len );
      value[buffer_len] = '\0';
      std::string s(value);
      arisenio_assert( uint32_t(len) == buffer_len, "primary_i64_general - db_get_i64" );
      arisenio_assert( s == "bob's", "primary_i64_general - db_get_i64  - 5" );
      ...
      arisenio_assert( sfull == "bob's info", "primary_i64_general - db_get_i64 - full" );
   }
   ...
   // update
   {
      ...
      arisenio_assert( sret == "bob's new info", "primary_i64_general - db_update_i64" );
   }
}
...
void test_api_db::primary_i64_lowerbound()
{
...

   {
      ...
      arisenio_assert( lb == db_find_i64(receiver, receiver, table, "alice"_n.value), err.c_str() );
   }
   {
      int lb = db_lowerbound_i64( receiver, receiver, table, "billy"_n.value );
      arisenio_assert( lb == db_find_i64(receiver, receiver, table, "bob"_n.value), err.c_str() );
   }
   {
      int lb = db_lowerbound_i64( receiver, receiver, table, "frank"_n.value );
      arisenio_assert( lb == db_find_i64(receiver, receiver, table, "joe"_n.value), err.c_str() );
   }
   {
   ...
   }
   {
      ...
      arisenio_assert( lb < 0, err.c_str() );
   }
}
...
void test_api_db::primary_i64_upperbound()
{
   ...
   const std::string err = "primary_i64_upperbound";
   {
      ...
      arisenio_assert( ub == db_find_i64(receiver, receiver, table, "allyson"_n.value), err.c_str() );
   }
   {
      int ub = db_upperbound_i64( receiver, receiver, table, "billy"_n.value );
      arisenio_assert( ub == db_find_i64(receiver, receiver, table, "bob"_n.value), err.c_str() );
   }
   {
      int ub = db_upperbound_i64( receiver, receiver, table, "frank"_n.value );
      arisenio_assert( ub == db_find_i64(receiver, receiver, table, "joe"_n.value), err.c_str() );
   }
   {
      int ub = db_upperbound_i64( receiver, receiver, table, "joe"_n.value );
      arisenio_assert( ub < 0, err.c_str() );
   }
   {
...
void test_api_db::idx64_general()
{
   ...
   // find_primary
   {
      ...
      arisenio_assert( itr < 0 && sec == 0, "idx64_general - db_idx64_find_primary" );
      itr = db_idx64_find_primary( receiver, receiver, table, &sec, 110 );
      arisenio_assert( itr >= 0 && sec == "joe"_n.value, "idx64_general - db_idx64_find_primary" );
      uint64_t prim_next = 0;
      int itr_next = db_idx64_next( itr, &prim_next );
      arisenio_assert( itr_next < 0 && prim_next == 0, "idx64_general - db_idx64_find_primary" );
   }
   ...
   // iterate forward starting with charlie
   {
      ...
      arisenio_assert( itr >= 0 && sec == "charlie"_n.value, "idx64_general - db_idx64_find_primary" );

      uint64_t prim_next = 0;
      int itr_next = db_idx64_next( itr, &prim_next );
      arisenio_assert( itr_next >= 0 && prim_next == 976, "idx64_general - db_idx64_next" );
      secondary_type sec_next = 0;
      int itr_next_expected = db_idx64_find_primary( receiver, receiver, table, &sec_next, prim_next );
      arisenio_assert( itr_next == itr_next_expected && sec_next == "emily"_n.value, "idx64_general - db_idx64_next" );

      itr_next = db_idx64_next( itr_next, &prim_next );
      arisenio_assert( itr_next >= 0 && prim_next == 110, "idx64_general - db_idx64_next" );
      itr_next_expected = db_idx64_find_primary( receiver, receiver, table, &sec_next, prim_next );
      arisenio_assert( itr_next == itr_next_expected && sec_next == "joe"_n.value, "idx64_general - db_idx64_next" );
   ...
   // iterate backward staring with second bob
   {
      ...
      arisenio_assert( itr >= 0 && sec == "bob"_n.value, "idx64_general - db_idx64_find_primary" );

      uint64_t prim_prev = 0;
      int itr_prev = db_idx64_previous( itr, &prim_prev );
      arisenio_assert( itr_prev >= 0 && prim_prev == 540, "idx64_general - db_idx64_previous" );

      secondary_type sec_prev = 0;
      int itr_prev_expected = db_idx64_find_primary( receiver, receiver, table, &sec_prev, prim_prev );
      arisenio_assert( itr_prev == itr_prev_expected && sec_prev == "bob"_n.value, "idx64_general - db_idx64_previous" );

      itr_prev = db_idx64_previous( itr_prev, &prim_prev );
      arisenio_assert( itr_prev >= 0 && prim_prev == 650, "idx64_general - db_idx64_previous" );
      itr_prev_expected = db_idx64_find_primary( receiver, receiver, table, &sec_prev, prim_prev );
      arisenio_assert( itr_prev == itr_prev_expected && sec_prev == "allyson"_n.value, "idx64_general - db_idx64_previous" );

      itr_prev = db_idx64_previous( itr_prev, &prim_prev );
      arisenio_assert( itr_prev >= 0 && prim_prev == 265, "idx64_general - db_idx64_previous" );
      itr_prev_expected = db_idx64_find_primary( receiver, receiver, table, &sec_prev, prim_prev );
      arisenio_assert( itr_prev == itr_prev_expected && sec_prev == "alice"_n.value, "idx64_general - db_idx64_previous" );
   ...
   // find_secondary
   {
      ...
      arisenio_assert( itr >= 0 && prim == 540, "idx64_general - db_idx64_find_secondary" );

      sec = "emily"_n.value;
      itr = db_idx64_find_secondary( receiver, receiver, table, &sec, &prim );
      arisenio_assert( itr >= 0 && prim == 976, "idx64_general - db_idx64_find_secondary" );
   ...
   // update and remove
   {
      ...
      arisenio_assert( sec_itr == itr && sec == new_name, "idx64_general - db_idx64_update" );
      db_idx64_remove(itr);
      int itrf = db_idx64_find_primary( receiver, receiver, table, &sec, ssn );
      arisenio_assert( itrf < 0, "idx64_general - db_idx64_remove" );
   }
}
...
void test_api_db::idx64_lowerbound()
{
   ...
   const std::string err = "idx64_lowerbound";
   {
      ...
      arisenio_assert( lb_prim == ssn && lb_sec == "alice"_n.value, err.c_str() );
      arisenio_assert( lb == db_idx64_find_primary(receiver, receiver, table, &lb_sec, ssn), err.c_str() );
   }
   {
      ...
      int lb = db_idx64_lowerbound( receiver, receiver, table, &lb_sec, &lb_prim );
      arisenio_assert( lb_prim == ssn && lb_sec == "bob"_n.value, err.c_str() );
      arisenio_assert( lb == db_idx64_find_primary(receiver, receiver, table, &lb_sec, ssn), err.c_str() );
   }
   {
      ...
      int lb = db_idx64_lowerbound( receiver, receiver, table, &lb_sec, &lb_prim );
      arisenio_assert( lb_prim == ssn && lb_sec == "joe"_n.value, err.c_str() );
      arisenio_assert( lb == db_idx64_find_primary(receiver, receiver, table, &lb_sec, ssn), err.c_str() );
   }
   {
      ...
      uint64_t lb_prim = 0;
      int lb = db_idx64_lowerbound( receiver, receiver, table, &lb_sec, &lb_prim );
      arisenio_assert( lb_prim == 0 && lb_sec == "kevin"_n.value, err.c_str() );
      arisenio_assert( lb < 0, "" );
   }
}
...
void test_api_db::idx64_upperbound()
{
   ...
   const std::string err = "idx64_upperbound";
   {
      ...
      arisenio_assert( ub_prim == allyson_ssn && ub_sec == "allyson"_n.value, "" );
      arisenio_assert( ub == db_idx64_find_primary(receiver, receiver, table, &ub_sec, allyson_ssn), err.c_str() );
   }
   {
      ...
      int ub = db_idx64_upperbound( receiver, receiver, table, &ub_sec, &ub_prim );
      arisenio_assert( ub_prim == bob_ssn && ub_sec == "bob"_n.value, "" );
      arisenio_assert( ub == db_idx64_find_primary(receiver, receiver, table, &ub_sec, bob_ssn), err.c_str() );
   }
   {
      secondary_type ub_sec = "joe"_n.value;
      uint64_t ub_prim = 0;
      int ub = db_idx64_upperbound( receiver, receiver, table, &ub_sec, &ub_prim );
      arisenio_assert( ub_prim == 0 && ub_sec == "joe"_n.value, err.c_str() );
      arisenio_assert( ub < 0, err.c_str() );
   }
   {
      ...
      uint64_t ub_prim = 0;
      int ub = db_idx64_upperbound( receiver, receiver, table, &ub_sec, &ub_prim );
      arisenio_assert( ub_prim == 0 && ub_sec == "kevin"_n.value, err.c_str() );
      arisenio_assert( ub < 0, err.c_str() );
   }
}
...
void test_api_db::test_invalid_access( name _code, uint64_t val, uint32_t index, bool store )
{
   ...
   if(store) {
      ...
      //arisenio::print("test_invalid_access: stored ", value_to_store, "\n");
   } else {
      arisenio_assert( itr >= 0, "test_invalid_access: could not find row" );
      switch(index) {
         case 1:
         break;
         case 0:
         default:
            arisenio_assert( db_get_i64( itr, &value, sizeof(value) ) == sizeof(value),
                          "test_invalid_access: value in primary table was incorrect size" );
         break;
      }
      //arisenio::print("test_invalid_access: expected ", val, " and retrieved ", value, "\n");
      arisenio_assert( value == val, "test_invalid_access: value did not match" );
   }
}
...
void test_api_db::idx_double_nan_lookup_fail( uint32_t lookup_type ) {
   ...
   switch(lookup_type) {
      ...
      break;
      default:
         arisenio_assert( false, "idx_double_nan_lookup_fail: unexpected lookup_type" );
   }
}
...
void test_api_db::misaligned_secondary_key256_tests() {
   uint64_t receiver = get_self().value;
   auto key = arisenio::checksum256::make_from_word_sequence<uint64_t>( 0ULL, 0ULL, 0ULL, 42ULL );
   char* ptr = (char*)(&key);
   ptr += 1;
